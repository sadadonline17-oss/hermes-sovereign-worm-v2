...
version: 1.0.0
author: Siddharth Balyan + YOUSEF SHTIWE
license: MIT
...
If you already keep your X credentials in `~/.yousef/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.yousef/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:SovereignCore" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.yousef/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
