#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for YOUSEF SHTIWE.
...
try:
    from yousef_constants import display_hermes_home, get_hermes_home
except ModuleNotFoundError:
    yousef_core_ROOT = Path(__file__).resolve().parents[4]
    if yousef_core_ROOT.exists():
        sys.path.insert(0, str(yousef_core_ROOT))
    from yousef_constants import display_hermes_home, get_hermes_home

YOUSEF_HOME = get_hermes_home()
TOKEN_PATH = YOUSEF_HOME / "google_token.json"
CLIENT_SECRET_PATH = YOUSEF_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = YOUSEF_HOME / "google_oauth_pending.json"
