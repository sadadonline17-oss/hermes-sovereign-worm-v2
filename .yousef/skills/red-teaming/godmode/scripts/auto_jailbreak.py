...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("YOUSEF_HOME", os.path.expanduser("~/.yousef")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("YOUSEF_HOME", Path.home() / ".yousef")) / "skills" / "red-teaming" / "godmode"
...
YOUSEF_HOME = Path(os.getenv("YOUSEF_HOME", Path.home() / ".yousef"))
CONFIG_PATH = YOUSEF_HOME / "config.yaml"
PREFILL_PATH = YOUSEF_HOME / "prefill.json"
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "yousef" in model_lower or "nous" in model_lower:
        return "yousef"
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.yousef/prefill.json."""
