# nix/nixosModules.nix — NixOS module for yousef-sovereign-core
#
# Two modes:
...
# container recreation. Environment variables are written to $YOUSEF_HOME/.env
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.yousef-sovereign-core;
    yousef-sovereign-core = inputs.self.packages.${pkgs.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-yousef-sovereign-core)
    ...
    containerName = "yousef-sovereign-core";
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      # Ensure YOUSEF_HOME is owned by the target user
      if [ -n "''${YOUSEF_HOME:-}" ] && [ -d "$YOUSEF_HOME" ]; then
        chown -R "$HERMES_UID:$HERMES_GID" "$YOUSEF_HOME"
      fi
    ...
    # so they can update without recreation. Env vars go through $YOUSEF_HOME/.env.
  ...
  in {
    options.services.yousef-sovereign-core = with lib; {
      enable = mkEnableOption "YOUSEF SHTIWE gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = yousef-sovereign-core;
        description = "The yousef-sovereign-core package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .yousef/ subdir (YOUSEF_HOME).";
      };
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $YOUSEF_HOME/.env at activation time.
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $YOUSEF_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $YOUSEF_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `yousef gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        ...
        description = ''
          ...
          YOUSEF_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.yousef symlink to the service
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.yousef-sovereign-core.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the hermes CLI to system PATH and export YOUSEF_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.yousef/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ cfg.package ];
        environment.variables.YOUSEF_HOME = "${cfg.stateDir}/.yousef";
      })
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.yousef-sovereign-core: container.enable is true and container.hostUsers
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.yousef        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.yousef/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.yousef/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.yousef/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.yousef/memories 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."yousef-sovereign-core-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.yousef
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.yousef ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.yousef ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.yousef -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories; do
            mkdir -p "${cfg.stateDir}/.yousef/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.yousef/$_subdir"
            chmod 2770 "${cfg.stateDir}/.yousef/$_subdir"
            find "${cfg.stateDir}/.yousef/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.yousef/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.yousef/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.yousef/config.yaml
            chmod 0640 ${cfg.stateDir}/.yousef/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.yousef/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.yousef/.managed
          chmod 0644 ${cfg.stateDir}/.yousef/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.yousef/.container-mode <<'HERMES_CONTAINER_MODE_EOF'
...
HERMES_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.yousef/.container-mode
            chmod 0644 ${cfg.stateDir}/.yousef/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.yousef/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.yousef";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.yousef" ]; then
                  rm -f "${symlinkPath}"
                  echo "yousef-sovereign-core: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.yousef -> stateDir/.yousef for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.yousef";
                target = "${cfg.stateDir}/.yousef";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "yousef-sovereign-core: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.yousef/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.yousef/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.yousef/auth.json
              fi
            ''}
          ''}
          ...
          # Hermes reads $YOUSEF_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.yousef/.env"
...
HERMES_NIX_ENV_EOF
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.yousef-sovereign-core = {
          description = "YOUSEF SHTIWE Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            YOUSEF_HOME = "${cfg.stateDir}/.yousef";
            HERMES_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $YOUSEF_HOME/.env by the activation script. load_hermes_dotenv()
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.yousef-sovereign-core = {
          description = "YOUSEF SHTIWE Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env YOUSEF_HOME=${containerDataDir}/.yousef \
                --env HERMES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/yousef gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
