...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.yousef/.env manually."
)
...
# Default location: {YOUSEF_HOME}/cache/images/ (legacy: image_cache/)
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; YousefShtiwe/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
