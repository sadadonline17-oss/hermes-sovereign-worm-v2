...
The PID file lives at ``{YOUSEF_HOME}/gateway.pid``.  YOUSEF_HOME defaults to
``~/.yousef`` but can be overridden via the environment variable.  This means
separate YOUSEF_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting YOUSEF_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "yousef" / _LOCKS_DIRNAME
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different YOUSEF_HOME dirs).
    """
