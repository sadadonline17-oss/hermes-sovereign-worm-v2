...
The ACP adapter wraps Sovereign' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
- `allow_once` -> Sovereign `once`
- `allow_always` -> Sovereign `always`
- reject options -> Sovereign `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Sovereign tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Sovereign' runtime resolver:

- `acp_adapter/auth.py`
- `yousef_shtiwe_cli_core/runtime_provider.py`

So ACP advertises and uses the currently configured Sovereign provider/credentials.
...
- `yousef_shtiwe_cli_core/main.py` — `hermes acp` CLI subcommand
