...
description: "How Sovereign stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `yousef_shtiwe_cli_core/cron.py` | CLI `hermes cron` subcommands |
...
Jobs are stored in `~/.yousef/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
```python
# ~/.yousef/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.yousef/cron/output/` |
...
- [Cron Feature Guide](/docs/mission-guide/features/cron)
