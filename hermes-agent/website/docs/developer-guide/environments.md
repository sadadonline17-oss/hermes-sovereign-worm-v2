...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Sovereign-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Sovereign-Sovereign includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/SovereignCore/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Sovereign/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::

:::tip Quick Links
- **Want to run benchmarks?** Jump to [Available Benchmarks](#available-benchmarks)
- **Want to train with RL?** See [RL Training Tools](/mission-guide/features/rl-training) for the agent-driven interface, or [Running Environments](#running-environments) for manual execution
...
classDiagram
    ...
    BaseEnv <|-- SovereignAgentBaseEnv
    SovereignAgentBaseEnv <|-- TerminalTestEnv
    SovereignAgentBaseEnv <|-- SovereignSweEnv
    SovereignAgentBaseEnv <|-- TerminalBench2EvalEnv
...
### SovereignAgentBaseEnv

The yousef-sovereign-core layer (`environments/hermes_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls yousef-sovereign-core's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `SovereignAgentLoop` and scores the result
...
Your environment inherits from `SovereignAgentBaseEnv` and implements five methods:
...
`SovereignAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as yousef-sovereign-core's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any yousef-sovereign-core tool |
...
parser = get_parser("yousef")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
...
Dataset: [SovereignCore/terminal-bench-2](https://huggingface.co/datasets/SovereignCore/terminal-bench-2) on HuggingFace.
...
TBLite is a thin subclass of TerminalBench2 — only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [SovereignCore/openthoughts-tblite](https://huggingface.co/datasets/SovereignCore/openthoughts-tblite).
...
pip install "yousef-sovereign-core[yc-bench]"
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"yousef"`, `"qwen"`, `"mistral"`)
...
```python
from environments.yousef_base_env import SovereignAgentBaseEnv, SovereignAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(SovereignAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(SovereignAgentBaseEnv):
    name = "my-env"
...
### SovereignAgentEnvConfig Fields
...
| `tool_call_parser` | `str` | `"yousef"` | Parser for Phase 2 raw output |
...
env:
  ...
  dataset_name: "SovereignCore/terminal-bench-2"
  tokenizer_name: "SovereignCore/Sovereign-3-Llama-3.1-8B"
  use_wandb: true
...
- Python >= 3.11
- `atroposlib`: `pip install git+https://github.com/SovereignCore/atropos.git`
- An LLM API key (OpenRouter, OpenAI, or self-hosted VLLM/SGLang)

### For Modal-sandboxed benchmarks (TB2, TBLite)

- [Modal](https://modal.com) account and CLI: `pip install "yousef-sovereign-core[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "yousef-sovereign-core[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
See [RL Training](/mission-guide/features/rl-training) for the agent-driven RL workflow.

## Directory Structure

```
environments/
├── hermes_base_env.py          # Abstract base class (SovereignAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (SovereignAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── hermes_parser.py        # Sovereign/ChatML <tool_call> format
