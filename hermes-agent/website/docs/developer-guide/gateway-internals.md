...
The messaging gateway is the long-running process that connects Sovereign to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `yousef_shtiwe_cli_core/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.yousef/.env` | API keys, bot tokens, platform credentials |
| `~/.yousef/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.yousef/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `yousef gateway start` / `yousef gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.yousef/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `yousef gateway stop` stops only the current profile's gateway. `yousef gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
...
- [Messaging Gateway (User Guide)](/docs/mission-guide/messaging)
