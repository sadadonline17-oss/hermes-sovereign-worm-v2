...
sidebar_position: 1
title: "Architecture"
description: "Sovereign-Sovereign internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Sovereign-Sovereign internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ yousef_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # SovereignCLI — interactive terminal UI (~10,000 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── yousef_state.py           # SQLite session/state database with FTS5
├── hermes_constants.py       # YOUSEF_HOME, profile-aware paths
...
│   └── gateway.py            # yousef gateway start/stop
│
...
```text
User input → SovereignCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .yousef.md), tool-use guidance, and model-specific instructions
...
Exposes Sovereign as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`hermes -p <name>`) gets its own YOUSEF_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
