...
description: "How to add a new inference provider to Sovereign-Sovereign — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Sovereign can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `yousef model` menu entries
...
1. `yousef_shtiwe_cli_core/auth.py` decides how credentials are found.
2. `yousef_shtiwe_cli_core/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `yousef_shtiwe_cli_core/models.py` and `yousef_shtiwe_cli_core/main.py` make the provider show up in the CLI. (`yousef_shtiwe_cli_core/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `yousef_shtiwe_cli_core/auth.py`
2. `yousef_shtiwe_cli_core/models.py`
3. `yousef_shtiwe_cli_core/runtime_provider.py`
4. `yousef_shtiwe_cli_core/main.py`
...
:::tip
`yousef_shtiwe_cli_core/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `yousef setup`.
:::
...
- `PROVIDER_REGISTRY` in `yousef_shtiwe_cli_core/auth.py`
- `_PROVIDER_LABELS` in `yousef_shtiwe_cli_core/models.py`
- `_PROVIDER_ALIASES` in both `yousef_shtiwe_cli_core/auth.py` and `yousef_shtiwe_cli_core/models.py`
- CLI `--provider` choices in `yousef_shtiwe_cli_core/main.py`
...
## Step 2: Add auth metadata in `yousef_shtiwe_cli_core/auth.py`
...
- What env vars should Sovereign check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `yousef_shtiwe_cli_core/models.py`
...
## Step 4: Resolve runtime data in `yousef_shtiwe_cli_core/runtime_provider.py`
...
Be careful with API-key precedence. Sovereign already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `yousef_shtiwe_cli_core/main.py`

A provider is not discoverable until it shows up in the interactive `yousef model` flow.

Update these in `yousef_shtiwe_cli_core/main.py`:
...
:::tip
`yousef_shtiwe_cli_core/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `yousef model` and `yousef setup` automatically.
:::
...
When you add a native provider, double-check that Sovereign is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m yousef_shtiwe_cli_core.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m yousef_shtiwe_cli_core.main model
python -m yousef_shtiwe_cli_core.main setup
```
...
- `website/docs/initial-infiltration/quickstart.md`
- `website/docs/mission-guide/configuration.md`
...
- [ ] `ProviderConfig` added in `yousef_shtiwe_cli_core/auth.py`
- [ ] aliases added in `yousef_shtiwe_cli_core/auth.py` and `yousef_shtiwe_cli_core/models.py`
- [ ] model catalog added in `yousef_shtiwe_cli_core/models.py`
- [ ] runtime branch added in `yousef_shtiwe_cli_core/runtime_provider.py`
- [ ] CLI wiring added in `yousef_shtiwe_cli_core/main.py` (setup.py inherits automatically)
...
### 7. Updating `yousef model` but not `yousef setup`
