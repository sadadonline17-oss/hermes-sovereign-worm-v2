...
Sovereign-Sovereign uses a SQLite database (`~/.yousef/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `yousef_state.py`


## Architecture Overview

```
~/.yousef/state.db (SQLite, WAL mode)
...
db = SessionDB()                           # Default: ~/.yousef/state.db
...
Default path: `~/.yousef/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.yousef/` by default, or the value of `YOUSEF_HOME` environment variable.
