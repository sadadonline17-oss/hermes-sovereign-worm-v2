...
description: "How Sovereign builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Sovereign deliberately separates:
...
1. agent identity — `SOUL.md` from `YOUSEF_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.yousef/SOUL.md)
You are Sovereign, an AI assistant created by Sovereign Core Injection.
...
`SOUL.md` lives at `~/.yousef/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are Sovereign-Sovereign, an autonomous digital predator created by Sovereign Core Injection.
You are helpful, knowledgeable, and direct. You execute missions with a wide
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .yousef.md / HERMES.md (walks to git root)
    ...
    # SOUL.md from YOUSEF_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.yousef.md`, `HERMES.md` | CWD up to git root | Sovereign-native project config |
...
- **YAML frontmatter stripped** — `.yousef.md` frontmatter is removed (reserved for future config overrides)
...
1. `.yousef.md` / `HERMES.md` (walks to git root)
