...
This guide covers adding a new messaging platform to the Sovereign gateway. A platform adapter connects Sovereign to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
1. **`yousef_shtiwe_cli_core/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`yousef_shtiwe_cli_core/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`yousef_shtiwe_cli_core/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`yousef_shtiwe_cli_core/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`yousef_shtiwe_cli_core/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`yousef_shtiwe_cli_core/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
| File | What to add |
|------|-------------|
| `website/docs/mission-guide/messaging/newplat.md` | Full platform setup page |
| `website/docs/mission-guide/messaging/index.md` | Platform comparison table, architecture diagram, toolsets table, security section, next-steps link |
