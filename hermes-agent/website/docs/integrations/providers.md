...
This page covers setting up inference providers for Sovereign-Sovereign — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Sovereign.

## Inference Providers

You need at least one way to connect to an LLM. Use `yousef model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `yousef model` (OAuth, subscription-based) |
| **OpenAI Codex** | `yousef model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `yousef model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `yousef model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `yousef model` (Claude Pro/Max via Claude Code auth, Anthropic API key, or manual setup-token) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.yousef/.env` |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.yousef/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.yousef/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.yousef/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.yousef/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.yousef/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.yousef/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.yousef/.env` (provider: `minimax-cn`) |
| **Alibaba Cloud** | `DASHSCOPE_API_KEY` in `~/.yousef/.env` (provider: `alibaba`, aliases: `dashscope`, `qwen`) |
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.yousef/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.yousef/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.yousef/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.yousef/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.yousef/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.yousef/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.yousef/.env` (provider: `gemini`) |
| **Google Gemini (OAuth)** | `yousef model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **Custom Endpoint** | `yousef model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
# → done — Sovereign auto-provisions your free tier on first request
```

Sovereign ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.yousef/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
...
- Responses translated back to OpenAI shape so the rest of Sovereign works unchanged
...
| VPC-SC-protected org | Sovereign detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Sovereign shows
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Sovereign stores the resulting credentials in its own auth store under `~/.yousef/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model — by default Gemini Flash via OpenRouter. An `OPENROUTER_API_KEY` enables these tools automatically. You can also configure which model and provider these tools use — see [Auxiliary Models](/docs/mission-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/docs/mission-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. It's offered automatically during `yousef model` setup, or enable it later with `hermes tools`.
:::

### Two Commands for Model Management

Sovereign has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`yousef model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Sovereign chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `yousef model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `yousef model`, complete the provider setup, then start a new session.
...
yousef chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `yousef model`
# Sovereign will use Claude Code's credential store directly when available
yousef model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
yousef chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
yousef chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `yousef model`, Sovereign prefers Claude Code's own credential store over copying the token into `~/.yousef/.env`. That keeps refreshable Claude credentials refreshable.
...
Sovereign supports GitHub Copilot as a first-class provider with two modes:
...
```bash
yousef chat --provider copilot --model gpt-5.4
```
...
If no token is found, `yousef model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `yousef model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `yousef model` to authenticate via OAuth instead.
:::
...
```bash
yousef chat --provider copilot-acp --model copilot-acp
...
```bash
# z.ai / ZhipuAI GLM
yousef chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.yousef/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
yousef chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.yousef/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
yousef chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.yousef/.env

# MiniMax (global endpoint)
yousef chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.yousef/.env

# MiniMax (China endpoint)
yousef chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.yousef/.env

# Alibaba Cloud / DashScope (Qwen models)
yousef chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.yousef/.env

# Xiaomi MiMo
yousef chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.yousef/.env

# Arcee AI (Trinity models)
yousef chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.yousef/.env
```
...
When using the Z.AI / GLM provider, Sovereign automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) Prompt Caching

When using xAI as a provider (any base URL containing `x.ai`), Sovereign automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
yousef chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# Requires: HF_TOKEN in ~/.yousef/.env

# Short alias
yousef chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Sovereign-Sovereign works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Sovereign at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of Sovereign — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `yousef setup` or config migration. Use `yousef model` or edit `config.yaml` directly.
:::
...
:::warning yousef model vs /model
**`yousef model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Sovereign chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `yousef model`, set up the new provider, then start a new session.
:::
...
When switching providers, Sovereign persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Sovereign.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Sovereign) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `hermes` (Qwen 2.5, Sovereign 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure Sovereign to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Sovereign won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Sovereign 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Sovereign) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Sovereign-Sovereign requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Sovereign config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Sovereign config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Sovereign.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Sovereign inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Sovereign's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Sovereign thinks the context is
...
Sovereign auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Sovereign uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Sovereign with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/mission-guide/configuration#context-compression) in Sovereign.
...
Then configure Sovereign with `yousef model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Sovereign with `yousef model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `yousef model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Sovereign uses this to decide when to compress history and to validate API requests.
...
Sovereign uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`yousef model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Sovereign uses "no-key-required" for keyless local servers
  - name: work
...
You can also select named custom providers from the interactive `yousef model` menu.
...
:::tip
You can switch between providers at any time with `yousef model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Sovereign at your instance (no API key needed):
   ```bash
   yousef config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.yousef/config.yaml`:
...
Configure a backup provider:model that Sovereign switches to automatically when your primary model fails (rate limits, server errors, auth failures):
...
:::tip
Fallback is configured exclusively through `config.yaml` — there are no environment variables for it. For full details on when it triggers, supported providers, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/docs/mission-guide/features/fallback-providers).
:::

## Smart Model Routing

Optional cheap-vs-strong routing lets Sovereign keep your main model for complex work while sending very short/simple turns to a cheaper model.
...
How it works:
- If a turn is short, single-line, and does not look code/tool/debug heavy, Sovereign may route it to `cheap_model`
- If the turn looks complex, Sovereign stays on your primary model/provider
- If the cheap route cannot be resolved cleanly, Sovereign falls back to the primary model automatically
...
- [Configuration](/docs/mission-guide/configuration) — General configuration (directory structure, config precedence, terminal backends, memory, compression, and more)
