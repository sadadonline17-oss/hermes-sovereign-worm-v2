...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Sovereign new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Sovereign how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.

For the full technical reference, see [Skills System](/docs/mission-guide/features/skills).

---

## Finding Skills

Every Sovereign installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Sovereign to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Sovereign but aren't active by default. Install them explicitly:
...
What happens:
1. The skill directory is copied to `~/.yousef/skills/`
...
For how to ship skills in your own plugin, see [Build a Sovereign Plugin → Bundle skills](/docs/guides/build-a-hermes-plugin#bundle-skills).
...
When a skill with config is first loaded, Sovereign prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.
...
yousef config get skills.config
```
...
```bash
mkdir -p ~/.yousef/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.yousef/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
yousef chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.yousef/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Sovereign may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Sovereign will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.yousef/skills/devops/`, `~/.yousef/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Sovereign to update the skill with what you learned. Skills that aren't maintained become liabilities.

---

*For the complete skills reference — frontmatter fields, conditional activation, external directories, and more — see [Skills System](/docs/mission-guide/features/skills).*
