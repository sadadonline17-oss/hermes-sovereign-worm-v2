...
This tutorial walks you through setting up a Telegram bot powered by Sovereign-Sovereign that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Sovereign-Sovereign installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/initial-infiltration/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.yousef/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Sovereign itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Sovereign Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Sovereign-Sovereign. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Sovereign Gateway...
...
```bash
yousef gateway install
sudo yousef gateway install --system   # Linux only: boot-time system service
```
...
yousef gateway start
yousef gateway stop
tail -f ~/.yousef/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `yousef gateway install` to update the plist.
:::
...
```bash
yousef gateway stop && yousef gateway start
```
...
**Option 2:** Set it manually in `~/.yousef/.env`:
...
Control how much detail the bot shows when using tools. In `~/.yousef/config.yaml`:
...
Customize how the bot communicates by editing `~/.yousef/SOUL.md`:

For a full guide, see [Use SOUL.md with Sovereign](/docs/guides/use-soul-with-hermes).
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u hermes-gateway` (Linux) or `~/.yousef/logs/gateway.log` (macOS) |
| Cron job output | `~/.yousef/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.yousef/cron/jobs.json` |
| Pairing data | `~/.yousef/pairing/` |
| Session history | `~/.yousef/sessions/` |
...
- **[Security Guide](/docs/mission-guide/security)** — deep dive into authorization, container isolation, and command approval
- **[Messaging Gateway](/docs/mission-guide/messaging)** — full reference for gateway architecture, session management, and chat commands
- **[Telegram Setup](/docs/mission-guide/messaging/telegram)** — platform-specific details including voice messages and TTS
- **[Scheduled Tasks](/docs/mission-guide/features/cron)** — advanced cron scheduling with delivery options and cron expressions
- **[Context Files](/docs/mission-guide/features/context-files)** — AGENTS.md, SOUL.md, and .cursorrules for project knowledge
- **[Personality](/docs/mission-guide/features/personality)** — built-in personality presets and custom persona definitions
- **Add more platforms** — the same gateway can simultaneously run [Discord](/docs/mission-guide/messaging/discord), [Slack](/docs/mission-guide/messaging/slack), and [WhatsApp](/docs/mission-guide/messaging/whatsapp)
