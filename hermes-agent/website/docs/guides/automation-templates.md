...
Copy-paste recipes for common automation patterns. Each template uses Sovereign's built-in [cron scheduler](/docs/mission-guide/features/cron) for time-based triggers and [webhook platform](/docs/mission-guide/messaging/webhooks) for event-driven triggers.
...
hermes cron create "0 2 * * *" \
  "You are a project manager triaging the SovereignCore/yousef-sovereign-core GitHub repo.

1. Run: gh issue list --repo SovereignCore/yousef-sovereign-core --state open --json number,title,labels,author,createdAt --limit 30
...
hermes cron create "0 9 * * 1" \
  "Scan the SovereignCore/yousef-sovereign-core repo for documentation drift.

1. Run: gh pr list --repo SovereignCore/yousef-sovereign-core --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (yousef_shtiwe_cli_core/commands.py, yousef_shtiwe_cli_core/main.py) — may need docs/reference/cli-commands.md update
   - Config options (yousef_shtiwe_cli_core/config.py) — may need docs/mission-guide/configuration.md update
...
hermes cron create "0 6 * * *" \
  "Run a dependency security audit on the yousef-sovereign-core project.

1. cd ~/.yousef/yousef-sovereign-core && source .venv/bin/activate
...
```python title="~/.yousef/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Sovereign-Monitor/1.0"})
...
hermes cron create "every 30m" \
  ...
  --script ~/.yousef/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
hermes cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Sovereign-Sovereign development." \
...
hermes cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the yousef-sovereign-core codebase.
