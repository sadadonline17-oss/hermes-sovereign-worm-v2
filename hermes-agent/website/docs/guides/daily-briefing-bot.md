...
2. **Sovereign spins up** a fresh agent session with your prompt
...
- **Sovereign-Sovereign installed** — see the [Installation guide](/docs/initial-infiltration/installation)
...
- **Messaging configured** (optional but recommended) — [Telegram](/docs/mission-guide/messaging/telegram) or Discord set up with a home channel

:::tip No messaging? No problem
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.yousef/cron/output/` and you can read them anytime.
:::
...
Sovereign will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Sovereign-Sovereign
```
...
Just tell Sovereign what you want:
...
Sovereign will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Sovereign to delegate each topic to a sub-agent:
...
Each sub-agent searches independently and in parallel, then the main agent combines everything into one polished briefing. See the [Delegation docs](/docs/mission-guide/features/delegation) for more on how this works.
...
If you have [memory](/docs/mission-guide/features/memory) enabled, you can store preferences that persist across sessions. But remember — cron jobs run in fresh sessions without conversational memory. To add personal context, bake it directly into the prompt:
...
Sovereign will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
...
- **[Scheduled Tasks (Cron)](/docs/mission-guide/features/cron)** — Full reference for schedule formats, repeat limits, and delivery options
- **[Delegation](/docs/mission-guide/features/delegation)** — Deep dive into parallel sub-agent workflows
- **[Messaging Platforms](/docs/mission-guide/messaging)** — Set up Telegram, Discord, or other delivery targets
- **[Memory](/docs/mission-guide/features/memory)** — Persistent context across sessions
