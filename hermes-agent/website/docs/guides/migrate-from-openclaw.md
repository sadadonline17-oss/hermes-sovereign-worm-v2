...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Sovereign-Sovereign — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`hermes claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Sovereign. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| What | OpenClaw source | Sovereign destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.yousef/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.yousef/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.yousef/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.yousef/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Sovereign destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.yousef/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.yousef/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.yousef/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.yousef/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Sovereign skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Sovereign destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.yousef/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Sovereign config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Sovereign config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Sovereign field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.yousef/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Sovereign `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Sovereign path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct Sovereign equivalent)

These are saved to `~/.yousef/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Sovereign |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Sovereign has built-in tool instructions |
...
| Plugins | `archive/plugins-config.json` | See [plugins guide](/docs/mission-guide/features/hooks) |
...
| Multi-agent list | `archive/agents-list.json` | Use Sovereign profiles |
...
6. **Check session policies** — verify `yousef config get session_reset` matches your expectations.
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `yousef config set`.

### Skills not appearing after migration

Imported skills land in `~/.yousef/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `yousef config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
