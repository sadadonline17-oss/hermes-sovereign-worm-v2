...
description: "Real-world automation patterns using Sovereign cron — monitoring, reports, pipelines, and multi-skill workflows"
---
...
For the full feature reference, see [Scheduled Tasks (Cron)](/docs/mission-guide/features/cron).
...
```bash
mkdir -p ~/.yousef/scripts
```

```python title="~/.yousef/scripts/watch-site.py"
import hashlib, json, os, urllib.request

URL = "https://example.com/pricing"
STATE_FILE = os.path.expanduser("~/.yousef/scripts/.watch-site-state.json")

# Fetch current content
req = urllib.request.Request(URL, headers={"User-Agent": "Sovereign-Monitor/1.0"})
...
```bash
/cron add "every 1h" "If the script output says CHANGE DETECTED, summarize what changed on the page and why it might matter. If it says NO_CHANGE, respond with just [SILENT]." --script ~/.yousef/scripts/watch-site.py --name "Pricing monitor" --deliver telegram
```
...
```bash
/cron add "every 6h" "Check the GitHub repository SovereignCore/yousef-sovereign-core for:
- New issues opened in the last 6 hours
- New PRs opened or merged in the last 6 hours
- Any new releases

Use the terminal to run gh commands:
  gh issue list --repo SovereignCore/yousef-sovereign-core --state open --json number,title,author,createdAt --limit 10
  gh pr list --repo SovereignCore/yousef-sovereign-core --state all --json number,title,author,createdAt,mergedAt --limit 10
...
```python title="~/.yousef/scripts/collect-prices.py"
import json, os, urllib.request
from datetime import datetime

DATA_DIR = os.path.expanduser("~/.yousef/data/prices")
...
If there's a significant move, explain what happened." \
  --script ~/.yousef/scripts/collect-prices.py \
  --name "Price tracker" \
  --deliver telegram
```
...
*For the complete cron reference — all parameters, edge cases, and internals — see [Scheduled Tasks (Cron)](/docs/mission-guide/features/cron).*
