...
description: "Frequently asked questions and solutions to common issues with Sovereign-Sovereign"
---
...
### What LLM providers work with Sovereign?

Sovereign-Sovereign works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** — Sovereign Core Injection's own inference endpoint
...
Set your provider with `yousef model` or by editing `~/.yousef/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Sovereign-Sovereign requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Sovereign from inside it. The standard install command works perfectly in WSL2:
...
Yes — Sovereign now has a tested Termux install path for Android phones.
...
For the fully explicit manual steps, supported extras, and current limitations, see the [Termux guide](../initial-infiltration/termux.md).
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Sovereign-Sovereign does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.yousef/`.

### Can I use it offline / with local models?

Yes. Run `yousef model`, select **Custom endpoint**, and enter your server's URL:
...
Sovereign persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.

This works with Ollama, vLLM, llama.cpp server, SGLang, LocalAI, and others. See the [Configuration guide](../mission-guide/configuration.md) for details.

:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Sovereign — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Sovereign auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `HERMES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Sovereign-Sovereign itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../mission-guide/messaging/index.md) lets multiple users interact with the same Sovereign-Sovereign instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Both persist across sessions. See [Memory](../mission-guide/features/memory.md) and [Skills](../mission-guide/features/skills.md) for details.

### Can I use it in my own Python project?

Yes. Import the `AIAgent` class and use Sovereign programmatically:
...
See the [Python Library guide](../mission-guide/features/code-execution.md) for full API usage.
...
**Cause:** Sovereign requires Python 3.11 or newer.
...
**Solution:** Exit your session and use `yousef model` from your terminal to add new providers:

```bash
# Exit the Sovereign chat session first (Ctrl+C or /quit)
...
After adding a new provider via `yousef model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `yousef model` (from terminal) |
| Enter/change API keys | `yousef model` (from terminal) |
...
# Or set directly
yousef config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.yousef/.env` for conflicting entries.
:::
...
# Set a valid model
yousef config set YOUSEF_SHTIWE_MODEL openrouter/nous/hermes-3-llama-3.1-70b

# Or specify per-session
yousef chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `yousef chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Sovereign detected the wrong context length for your model.
...
yousef chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Sovereign may have the wrong context length for your model. Check what it detected:
...
**Cause:** Sovereign detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.

**Solution:** When prompted, review the command and type `y` to approve it. You can also:
- Ask the agent to use a safer alternative
- See the full list of dangerous patterns in the [Security docs](../mission-guide/security.md)

:::tip
This is working as intended — Sovereign never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `yousef chat`
...
yousef gateway status

# Start the gateway
yousef gateway start

# Check logs for errors
cat ~/.yousef/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `yousef gateway setup`
- Check gateway logs: `cat ~/.yousef/logs/gateway.log | tail -50`
...
Configure in `~/.yousef/config.yaml` under your gateway's settings. See the [Messaging docs](../mission-guide/messaging/index.md).
...
pip install "yousef-sovereign-core[telegram]"   # or [discord], [slack], [whatsapp]
...
#### WSL: Gateway keeps disconnecting or `yousef gateway start` fails
...
tmux new -s hermes 'yousef gateway run'
# Reattach later: tmux attach -t hermes

# Option 3: Background via nohup
nohup yousef gateway run > ~/.yousef/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'yousef gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `yousef gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
yousef gateway install    # Re-snapshots your current PATH
yousef gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.yousef.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `yousef chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `yousef chat -t "terminal"`
...
cd ~/.yousef/yousef-sovereign-core && uv pip install -e ".[mcp]"
...
Verify your `~/.yousef/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
yousef config show | grep -A 12 mcp_servers

# Restart Sovereign or reload MCP after config changes
yousef chat
```

See also:
- [MCP (Model Context Protocol)](/docs/mission-guide/features/mcp)
- [Use MCP with Sovereign](/docs/guides/use-mcp-with-hermes)
...
:::warning
If an MCP server crashes mid-request, Sovereign will report a timeout. Check the server's own logs (not just Sovereign logs) to diagnose the root cause.
:::
...
There is no hard limit. Each profile is just a directory under `~/.yousef/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Sovereign can route subagents to a different model automatically. Set this in `~/.yousef/config.yaml`:
...
Now when you tell Sovereign "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
See [Subagent Delegation](../mission-guide/features/delegation.md) for more on how delegation works.

### Running multiple agents on one WhatsApp number (per-chat binding)

**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Sovereign do this?

**Current limitation:** Sovereign profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
See [Profiles](../mission-guide/profiles.md) and [WhatsApp setup](../mission-guide/messaging/whatsapp.md) for more details.

### Controlling what shows up in Telegram (hiding logs and reasoning)

**Scenario:** You see gateway exec logs, Sovereign reasoning, and tool call details in Telegram instead of just the final output.
...
After changing this, **restart the gateway** (`yousef gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Sovereign creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Sovereign to another machine
...
1. Install Sovereign-Sovereign on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/SovereignCore/yousef-sovereign-core/main/scripts/install.sh | bash
   ```

2. Copy your entire `~/.yousef/` directory **except** the `yousef-sovereign-core` subdirectory (that's the code repo — the new install has its own):
   ```bash
   # On the source machine
   rsync -av --exclude='yousef-sovereign-core' ~/.yousef/ newmachine:~/.yousef/
   ```
...
3. On the new machine, run `yousef setup` to verify API keys and provider config are working. Re-authenticate any messaging platforms (especially WhatsApp, which uses QR pairing).

The `~/.yousef/` directory contains everything: `config.yaml`, `.env`, `SOUL.md`, `memories/`, `skills/`, `state.db` (sessions), `cron/`, and any custom plugins. The code itself lives in `~/.yousef/yousef-sovereign-core/` and is installed fresh.

### Permission denied when reloading shell after install

**Scenario:** After running the Sovereign installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Sovereign-specific issue — it's a shell config permissions problem.
...
yousef config show | head -20

# Re-run model selection
yousef model

# Or test with a known-good model
yousef chat -q "hello" --model anthropic/claude-sonnet-4.6
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/SovereignCore/yousef-sovereign-core/issues)
2. **Ask the community:** [Sovereign Core Injection Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Sovereign version (`hermes --version`), and the full error message
