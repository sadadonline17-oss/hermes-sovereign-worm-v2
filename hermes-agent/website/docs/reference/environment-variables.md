...
description: "Complete reference of all environment variables used by Sovereign-Sovereign"
---

# Environment Variables Reference

All variables go in `~/.yousef/.env`. You can also set them with `yousef config set VAR value`.
...
| `YOUSEF_SHTIWE_MODEL` | Override model name at process level (used by cron scheduler; prefer `config.yaml` for normal use) |
...
| `YOUSEF_HOME` | Override Sovereign config directory (default: `~/.yousef`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |

## Provider Auth (OAuth)

For native Anthropic auth, Sovereign prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Pro/Max login.
...
These variables configure the [Tool Gateway](/docs/mission-guide/features/tool-gateway) for paid Nous subscribers or self-hosted gateway deployments. Most users don't need to set these — the gateway is configured automatically via `yousef model` or `hermes tools`.
...
For cloud sandbox backends, persistence is filesystem-oriented. `TERMINAL_LIFETIME_SECONDS` controls when Sovereign cleans up an idle terminal session, and later resumes may recreate the sandbox rather than keep the same live processes running.
...
| `TERMINAL_SANDBOX_DIR` | Host directory for workspaces and overlays (default: `~/.yousef/sandboxes/`) |
...
| `DISCORD_ALLOW_MENTION_EVERYONE` | Allow the bot to ping `@everyone`/`@here` (default: `false`). See [Mention Control](../mission-guide/messaging/discord.md#mention-control). |
...
| `API_SERVER_MODEL_NAME` | Model name advertised on `/v1/models`. Defaults to the profile name (or `yousef-sovereign-core` for the default profile). Useful for multi-user setups where frontends like Open WebUI need distinct model names per connection. |
| `GATEWAY_PROXY_URL` | URL of a remote Sovereign API server to forward messages to ([proxy mode](/docs/mission-guide/messaging/matrix#proxy-mode-e2ee-on-macos)). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Also configurable via `gateway.proxy_url` in `config.yaml`. |
...
| `YOUSEF_QUIET` | Suppress non-essential output (`true`/`false`) |
...
| `HERMES_ENABLE_PROJECT_PLUGINS` | Enable auto-discovery of repo-local plugins from `./.yousef/plugins/` (`true`/`false`, default: `false`) |
...
For task-specific direct endpoints, Sovereign uses the task's configured API key or `OPENAI_API_KEY`. It does not reuse `OPENROUTER_API_KEY` for those custom endpoints.
...
See [Fallback Providers](/docs/mission-guide/features/fallback-providers) for full details.

## Provider Routing (config.yaml only)

These go in `~/.yousef/config.yaml` under the `provider_routing` section:
...
:::tip
Use `yousef config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
