...
title: "Git Worktrees"
description: "Run multiple Sovereign agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Sovereign-Sovereign is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Sovereign so each session has a clean, isolated working directory.

## Why Use Worktrees with Sovereign?

Sovereign treats the **current working directory** as the project root:

- CLI: the directory where you run `hermes` or `yousef chat`
...
Now you can `cd` into the new worktree and run Sovereign there:

```bash
cd ../repo-feature

# Start Sovereign in the worktree
hermes
```
...
- Sovereign checkpoint data under `~/.yousef/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Sovereign experiment**
...
- **Avoid running Sovereign from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `hermes -w` (Automatic Worktree Mode)

Sovereign has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Use **git worktrees** to give each Sovereign session its own clean checkout.
