...
1. Enable via `yousef gateway setup` or environment variables
...
Add the `github-pr` route to your `~/.yousef/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Sovereign processes the event, and posts a review comment on the PR.
...
- Subscriptions are stored in `~/.yousef/webhook_subscriptions.json`
...
- Run the gateway in foreground to see logs: `yousef gateway run`
