...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Sovereign-Sovereign as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Sovereign-Sovereign integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Sovereign opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Sovereign responds to every message. |
| Group chats | Sovereign responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Sovereign will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `yousef gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Sovereign runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Sovereign behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Sovereign starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Sovereign responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Sovereign responds to @mentions from any user in any group. |
| `allowlist` | Sovereign only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Sovereign ignores all group messages entirely. |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.yousef/feishu_seen_message_ids.json`.
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `yousef gateway setup` |
| `Another local Sovereign gateway is already using this Feishu app_id` | Only one Sovereign instance can use the same app_id at a time. Stop the other gateway first. |
