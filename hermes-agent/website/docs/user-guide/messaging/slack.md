...
sidebar_position: 4
title: "Slack"
description: "Set up Sovereign-Sovereign as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Sovereign-Sovereign to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Sovereign instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Sovereign uses the modern
...
4. Enter an app name (e.g., "Sovereign-Sovereign") and select your workspace
5. Click **Create App**
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Sovereign configuration issue.
:::
...
Sovereign uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
yousef gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
yousef gateway              # Foreground
yousef gateway install      # Install as a user service
...
Understanding how Sovereign behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Sovereign-Sovereign what time is it?`). In channels, Sovereign replies in a thread attached to that message. |
| **Threads** | If you @mention Sovereign inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.yousef/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Sovereign in `#general` will have separate histories and contexts.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Sovereign will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Sovereign-Sovereign`).

---

## Multi-Workspace Support

Sovereign can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Sovereign also loads tokens from an **OAuth token file** at:

```
~/.yousef/slack_tokens.json
```
...
- When a message arrives, Sovereign uses the correct workspace-specific client to respond.
...
Sovereign supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Sovereign-Sovereign` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Sovereign-Sovereign` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Sovereign-Sovereign`)
...
- Tokens should be stored in `~/.yousef/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Sovereign config directory
