...
sidebar_position: 9
title: "Matrix"
description: "Set up Sovereign-Sovereign as a Matrix bot"
---

# Matrix Setup

Sovereign-Sovereign integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Sovereign-Sovereign pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Sovereign works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Sovereign behaves once it's connected.

## How Sovereign Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Sovereign responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Sovereign requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Sovereign supports Matrix threads (MSC3440). If you reply in a thread, Sovereign keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Sovereign auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, Sovereign isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
You can also run Sovereign as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Sovereign needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Sovereign the bot's user ID and password. Sovereign will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
Sovereign-Sovereign uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure Sovereign-Sovereign
...
Add the following to your `~/.yousef/.env` file:
...
Optional behavior settings in `~/.yousef/config.yaml`:
...
:::tip
You can run `yousef gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

Sovereign supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
pip install 'yousef-sovereign-core[matrix]'
```
...
Add to your `~/.yousef/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, Sovereign:

- Stores encryption keys in `~/.yousef/platforms/matrix/store/` (legacy installs: `~/.yousef/matrix/store/`)
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Sovereign imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

:::warning[Deleting the crypto store]
If you delete `~/.yousef/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

Sovereign detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
2. Delete the local crypto store and restart Sovereign:
   ```bash
   rm -f ~/.yousef/platforms/matrix/store/crypto.db*
   # restart hermes
   ```
...
If you previously used Sovereign with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       ...
       "password": "***",
       "initial_device_display_name": "Sovereign-Sovereign"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.yousef/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.yousef/platforms/matrix/store/crypto.db
   rm -f ~/.yousef/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.yousef/.env`:
...
a working E2EE setup with a previous version of Sovereign and are upgrading.
...
Matrix E2EE requires `libolm`, which doesn't compile on macOS ARM64 (Apple Silicon). The `yousef-sovereign-core[matrix]` extra is gated to Linux only. If you're on macOS, proxy mode lets you run E2EE in a Docker container on a Linux VM while the actual agent runs natively on macOS with full access to your local files, memory, and skills.
...
services:
  hermes-matrix:
    ...
    volumes:
      - ./matrix-store:/root/.yousef/platforms/matrix/store
```
...
RUN pip install 'yousef-sovereign-core[matrix]'
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Sovereign API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Sovereign-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Sovereign logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Sovereign gateway isn't running, or it failed to connect.

**Fix**: Check that `yousef gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.yousef/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your Sovereign-Sovereign deployment, see the [Security Guide](../security.md).
...
- **Media support**: Sovereign can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
