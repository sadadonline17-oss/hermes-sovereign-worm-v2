...
sidebar_position: 2
title: "Configuration"
description: "Configure Sovereign-Sovereign — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.yousef/` directory for easy access.
...
```bash
yousef config              # View current configuration
yousef config edit         # Open config.yaml in your editor
yousef config set KEY VAL  # Set a specific value
yousef config check        # Check for missing options (after updates)
yousef config migrate      # Interactively add missing options

# Examples:
yousef config set model anthropic/claude-opus-4
yousef config set terminal.backend docker
yousef config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `yousef config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `yousef chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.yousef/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.yousef/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Sovereign supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox, a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means Sovereign will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Sovereign probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle).
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.yousef/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.yousef/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.yousef/` (OAuth tokens, etc.) and synced before each command.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/yousef-sovereign-core` (HPC convention) → `~/.yousef/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `yousef config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Sovereign logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `yousef doctor` to check.
...
Sovereign resolves each listed variable from your current shell first, then falls back to `~/.yousef/.env` if it was saved with `yousef config set`.
...
Docker sandboxes stay isolated by default. Sovereign does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Sovereign from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Sovereign from
...
```bash
yousef config set terminal.persistent_shell false
```
...
- `yousef config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `yousef config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
yousef config set skills.config.myplugin.path ~/myplugin-data
```
...
Sovereign automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
See [Memory Providers](/docs/mission-guide/features/memory-providers) for the analogous single-select system for memory plugins.
...
The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Sovereign raises this to 30 minutes when it detects a local endpoint. If you explicitly set `HERMES_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
Options: `fill_first` (default), `round_robin`, `least_used`, `random`. See [Credential Pools](/docs/mission-guide/features/credential-pools) for full documentation.

## Auxiliary Models

Sovereign uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection — you don't need to configure anything.

### The universal config pattern

Every model slot in Sovereign — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Sovereign ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Sovereign uses that provider's built-in auth and base URL.
...
Or via environment variable (in `~/.yousef/.env`):
...
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `yousef model` → Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `yousef model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Sovereign uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` uses whatever provider Sovereign uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `yousef config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
display:
  ...
  skin: default           # Built-in or custom CLI skin (see mission-guide/features/skins)
...
`interim_assistant_messages` is gateway-only. When enabled, Sovereign sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, Sovereign falls back automatically in this order: `local` → `groq` → `openai`.
...
Use `/voice on` in the CLI to enable microphone mode, `record_key` to start/stop recording, and `/voice tts` to toggle spoken replies. See [Voice Mode](/docs/mission-guide/features/voice-mode) for end-to-end setup and platform-specific behavior.
...
:::note
Streaming is disabled by default. Enable it in `~/.yousef/config.yaml` to try the streaming UX.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Sovereign to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Sovereign still keys DMs by chat/DM ID as usual.
...
For the behavior details and examples, see [Sessions](/docs/mission-guide/sessions) and the [Discord guide](/docs/mission-guide/messaging/discord).

## Unauthorized DM Behavior

Control what Sovereign does when an unknown user sends a direct message:
...
- `pair` is the default. Sovereign denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  status:
    type: exec
    command: systemctl status yousef-sovereign-core
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.yousef/yousef-sovereign-core && git pull && pip install -e .
  gpu:
    type: exec
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.yousef/browser_recordings/
  camofox:
    managed_persistence: false   # When true, Camofox sessions persist cookies/logins across restarts
```

The browser toolset supports multiple providers. See the [Browser feature page](/docs/mission-guide/features/browser) for details on Browserbase, Browser Use, and local Chrome CDP setup.
...
Control how Sovereign handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `YOUSEF_YOLO_MODE=true`. **Use with caution.** |
...
Automatic filesystem snapshots before destructive file operations. See the [Checkpoints & Rollback](/docs/mission-guide/checkpoints-and-rollback) for details.
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Sovereign falls back to `OPENAI_API_KEY` only.
...
Sovereign uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.yousef/SOUL.md` or `$YOUSEF_HOME/SOUL.md` |
| `.yousef.md` / `HERMES.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, Sovereign falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.yousef.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Sovereign automatically seeds a default `SOUL.md` if one does not already exist.
- All loaded context files are capped at 20,000 characters with smart truncation.

See also:
- [Personality & SOUL.md](/docs/mission-guide/features/personality)
- [Context Files](/docs/mission-guide/features/context-files)
...
```bash
# In ~/.yousef/.env or ~/.yousef/config.yaml:
