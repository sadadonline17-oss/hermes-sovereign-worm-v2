...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Sovereign-Sovereign terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Sovereign-Sovereign's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
yousef chat --model "anthropic/claude-sonnet-4"

# With a specific provider
yousef chat --provider nous        # Use Nous Portal
yousef chat --provider openrouter  # Force OpenRouter
...
yousef chat -s github-pr-workflow -q "open a draft PR"
...
<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Sovereign CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Sovereign CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Sovereign to background (Unix only). Run `fg` in the shell to resume. |
| `Tab` | Accept auto-suggestion (ghost text) or autocomplete slash commands |

## Slash Commands

Type `/` to see the autocomplete dropdown. Sovereign supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Sovereign replies |
...
Then type `/status` or `/gpu` in any chat. See the [Configuration guide](/docs/mission-guide/configuration#quick-commands) for more examples.
...
```bash
hermes -s yousef-sovereign-core-dev,github-auth
yousef chat -s github-pr-workflow -s github-auth
```

Sovereign loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.yousef/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.yousef/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Sovereign..."
  # Add your own!
```
...
On Unix systems, press **`Ctrl+Z`** to suspend Sovereign to the background — just like any terminal process. The shell prints a confirmation:

```
Sovereign-Sovereign has been suspended. Run `fg` to bring Sovereign-Sovereign back.
```
...
Cycle through display modes with `/verbose`: `off → new → all → verbose`. This command can also be enabled for messaging platforms — see [configuration](/docs/mission-guide/configuration#display-settings).
...
CLI sessions are stored in Sovereign's SQLite state database under `~/.yousef/state.db`. The database keeps:
...
Sovereign immediately confirms the task and gives you back the prompt:
