...
sidebar_position: 7
title: "Docker"
description: "Running Sovereign-Sovereign in Docker and using Docker as a terminal backend"
---

# Sovereign-Sovereign — Docker

There are two distinct ways Docker intersects with Sovereign-Sovereign:

1. **Running Sovereign IN Docker** — the agent itself runs inside a container (this page's primary focus)
...
If this is your first time running Sovereign-Sovereign, create a data directory on the host and start the container interactively to run the setup wizard:

```sh
mkdir -p ~/.yousef
docker run -it --rm \
  -v ~/.yousef:/opt/data \
  nousresearch/yousef-sovereign-core setup
```

This drops you into the setup wizard, which will prompt you for your API keys and write them to `~/.yousef/.env`. You only need to do this once. It is highly recommended to set up a chat system for the gateway to work with at this point.
...
docker run -d \
  ...
  -v ~/.yousef:/opt/data \
  -p 8642:8642 \
  nousresearch/yousef-sovereign-core gateway run
```
...
docker run -d \
  ...
  nousresearch/yousef-sovereign-core dashboard
```
...
docker run -it --rm \
  -v ~/.yousef:/opt/data \
  nousresearch/yousef-sovereign-core
```

## Persistent volumes

The `/opt/data` volume is the single source of truth for all Sovereign state. It maps to your host's `~/.yousef/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All Sovereign configuration |
...
:::warning
Never run two Sovereign **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access. Running a dashboard container alongside the gateway is safe since the dashboard only reads data.
:::
...
services:
  hermes:
    image: nousresearch/yousef-sovereign-core:latest
...
The Sovereign container needs moderate resources. Recommended minimums:
...
- Python 3 with all Sovereign dependencies (`pip install -e ".[all]"`)
...
```sh
docker pull nousresearch/yousef-sovereign-core:latest
docker rm -f hermes
docker run -d \
  --name hermes \
  --restart unless-stopped \
...
The container runs as root by default. If your host `~/.yousef/` was created by a non-root user, permissions should work. If you get errors, ensure the data directory is writable:
...
docker run -it --rm nousresearch/yousef-sovereign-core:latest version     # Verify version
