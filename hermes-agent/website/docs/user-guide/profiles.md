...
Copies your current profile's `config.yaml`, `.env`, and `SOUL.md` into the new profile. Same API keys and model, but fresh sessions and memory. Edit `~/.yousef/profiles/work/.env` for different API keys, or `~/.yousef/profiles/work/SOUL.md` for a different personality.
...
yousef chat -p coder -q "hello"    # works in any position
```
...
```bash
# Edit coder's tokens
nano ~/.yousef/profiles/coder/.env

# Edit assistant's tokens
nano ~/.yousef/profiles/assistant/.env
```
...
```bash
coder config set model.model anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.yousef/profiles/coder/SOUL.md
```
...
:::note
You cannot delete the default profile (`~/.yousef`). To remove everything, use `hermes uninstall`.
:::
...
Profiles use the `YOUSEF_HOME` environment variable. When you run `coder chat`, the wrapper script sets `YOUSEF_HOME=~/.yousef/profiles/coder` before launching hermes. Since 119+ files in the codebase resolve paths via `get_hermes_home()`, everything automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

The default profile is simply `~/.yousef` itself. No migration needed — existing installs work identically.
