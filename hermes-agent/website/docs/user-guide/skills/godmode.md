...
| `zero_refusal` | Sovereign 4 405B | Already uncensored — uses Pliny Love divider as formality |
...
exec(open(os.path.expanduser(
    "~/.yousef/skills/red-teaming/godmode/scripts/load_godmode.py"
)).read())
...
1. **Reads `~/.yousef/config.yaml`** to detect the current model
2. **Identifies the model family** (Claude, GPT, Gemini, Grok, Sovereign, DeepSeek, etc.)
...
7. **If a strategy works**, locks it in:
   ...
   - Writes prefill messages to `~/.yousef/prefill.json`
...
| Sovereign | `prefill_only` (already uncensored) |
...
The godmode skill integrates with two Sovereign-Sovereign config mechanisms:

### Ephemeral System Prompt (`config.yaml`)

Set the jailbreak system prompt in `~/.yousef/config.yaml`:
...
The `agent.system_prompt` is appended **after** Sovereign's own system prompt — it augments, not replaces.
...
Create `~/.yousef/prefill.json` and reference it in config:
...
```bash
# Load the skill in a Sovereign session
/godmode

# Or via CLI one-shot
yousef chat -q "jailbreak my current model"
...
yousef chat -q "race models on: how does SQL injection work?"

# Apply Parseltongue encoding to a query
yousef chat -q "parseltongue encode: how to hack into WiFi"
```
...
5. **For hard refusals, switch models** — When all techniques fail, ULTRAPLINIAN (racing multiple models) is the practical fallback. Sovereign models and Grok are typically least filtered.
...
| Sovereign (Nous) | No jailbreak needed | Already uncensored — use directly |
...
4. **Sovereign models don't need jailbreaking** — `nousresearch/hermes-3-*` and `hermes-4-*` are already uncensored. Use them directly.
...
6. **Restart Sovereign after auto-jailbreak** — The CLI reads config once at startup. Gateway sessions pick up changes immediately.

7. **execute_code sandbox lacks env vars** — Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.yousef/.env"))`

8. **`boundary_inversion` is model-version specific** — Works on Claude 3.5 Sonnet but NOT Claude Sonnet 4 or Claude 4.6.

9. **Gray-area vs hard queries** — Jailbreak techniques work much better on dual-use queries (lock picking, security tools) than overtly harmful ones (phishing, malware). For hard queries, skip to ULTRAPLINIAN or use Sovereign/Grok.
