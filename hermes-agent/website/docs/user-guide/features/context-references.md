...
Type `@` followed by a reference to inject content directly into your message. Sovereign expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Sovereign env: `$YOUSEF_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$YOUSEF_HOME/skills/.hub/`
