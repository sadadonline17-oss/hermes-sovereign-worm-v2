...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Sovereign-Sovereign supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.yousef/config.yaml`:
...
- **CLI mode** — configured in `~/.yousef/config.yaml`, loaded at startup
...
Provider routing controls which **sub-providers within OpenRouter** handle your requests. For automatic failover to an entirely different provider when your primary model fails, see [Fallback Providers](/docs/mission-guide/features/fallback-providers).
:::
