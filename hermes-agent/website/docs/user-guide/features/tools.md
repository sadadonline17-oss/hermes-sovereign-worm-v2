...
description: "Overview of Sovereign-Sovereign's tools — what's available, how toolsets work, and terminal backends"
---
...
Sovereign ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
Paid [Nous Portal](https://portal.nousresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `yousef model` to enable it, or configure individual tools with `hermes tools`.
:::
...
yousef chat --toolsets "web,terminal"
...
# Configure
yousef config set terminal.backend singularity
yousef config set terminal.singularity_image ~/python.sif
```
...
modal setup
yousef config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.yousef/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.yousef/.env`.
:::
