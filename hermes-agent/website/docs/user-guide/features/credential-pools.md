...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Sovereign automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Sovereign auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:
...
When you set up a custom endpoint via `yousef model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via yousef model:
hermes auth list
# Shows:
...
Sovereign automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Sovereign PKCE OAuth | `~/.yousef/auth.json` | Yes (Anthropic) |
...
2. **`yousef_shtiwe_cli_core/auth_commands.py`** — CLI commands and interactive wizard
3. **`yousef_shtiwe_cli_core/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.yousef/auth.json` under the `credential_pool` key:
