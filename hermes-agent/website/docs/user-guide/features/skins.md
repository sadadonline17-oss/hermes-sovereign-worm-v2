...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Sovereign CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Sovereign CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.yousef/skins/mytheme.yaml
```

Or set the default skin in `~/.yousef/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Sovereign — gold and kawaii | `Sovereign-Sovereign` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Sovereign-Sovereign` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Sovereign-Sovereign` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Sovereign-Sovereign` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Sovereign-Sovereign` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
| `ui_label` | UI labels and tags | `#4dd0e1` (red) |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Sovereign-Sovereign` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Sovereign-Sovereign! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ☠` |
| `response_label` | Label on the response box header | ` ☠ WORM V2 ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default yousef_core banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.yousef/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.yousef/skins/mytheme.yaml
...
## Sovereign Mod — Visual Skin Editor

[Sovereign Mod](https://github.com/cocktailpeanut/hermes-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Sovereign Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/hermes-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Sovereign skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.yousef/skins/`
- Activates a skin by updating `~/.yousef/config.yaml`
...
6. Click **Save** to write the skin YAML to `~/.yousef/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Sovereign Mod respects the `YOUSEF_HOME` environment variable, so it works with [profiles](/docs/mission-guide/profiles) too.

## Operational notes

- Built-in skins load from `yousef_shtiwe_cli_core/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.yousef/skins/` take precedence over built-in skins with the same name.
