...
When you run `yousef model` and select Nous Portal as your provider, Sovereign automatically offers to enable the Tool Gateway:
...
Set the `use_gateway` flag directly in `~/.yousef/config.yaml`:
...
The gateway authenticates using your Nous Portal credentials (stored in `~/.yousef/auth.json` after `yousef model`).
...
For self-hosted or custom gateway deployments, you can override the gateway endpoints via environment variables in `~/.yousef/.env`:
...
Modal (serverless terminal backend) is available as an optional add-on through the Nous subscription. It's not enabled by the Tool Gateway prompt — configure it separately via `yousef setup terminal` or in `config.yaml`.
