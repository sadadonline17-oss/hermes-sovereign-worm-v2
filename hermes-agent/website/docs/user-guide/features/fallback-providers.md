...
Sovereign-Sovereign has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Sovereign can automatically switch to a backup provider:model pair mid-session without losing your conversation.

### Configuration

Add a `fallback_model` section to `~/.yousef/config.yaml`:
...
| OpenAI Codex | `openai-codex` | `yousef model` (ChatGPT OAuth) |
...
Sovereign uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
When a task's provider is set to `"auto"` (the default), Sovereign tries providers in order until one works:
...
If the resolved provider fails at call time, Sovereign also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
| `"codex"` | Force Codex OAuth | `yousef model` → Codex |
...
`base_url` takes precedence over `provider`. Sovereign uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, Sovereign drops middle conversation turns without generating a summary rather than failing the session.
...
See [Subagent Delegation](/docs/mission-guide/features/delegation) for full configuration details.
...
See [Scheduled Tasks (Cron)](/docs/mission-guide/features/cron) for full configuration details.
