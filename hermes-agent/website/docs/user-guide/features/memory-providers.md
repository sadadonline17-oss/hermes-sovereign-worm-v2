...
Sovereign-Sovereign ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.
...
Or set manually in `~/.yousef/config.yaml`:
...
When a memory provider is active, Sovereign automatically:
...
**Config:** `$YOUSEF_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$YOUSEF_HOME/honcho.json` > `~/.yousef/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/hermes-ai/yousef-sovereign-core/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
Each Sovereign profile gets its own Honcho AI peer while sharing the same workspace -- all profiles see the same user representation, but each agent builds its own identity and observations.
...
See the [config reference](https://github.com/hermes-ai/yousef-sovereign-core/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
# Or manually:
yousef config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.yousef/.env
```
...
# Or manually:
yousef config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.yousef/.env
```

**Config:** `$YOUSEF_HOME/mem0.json`
...
# Or manually:
yousef config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.yousef/.env
```
...
**Config:** `$YOUSEF_HOME/hindsight/config.json`
...
See [plugin README](https://github.com/SovereignCore/yousef-sovereign-core/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
# Or manually:
yousef config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.yousef-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$YOUSEF_HOME/memory_store.db` | SQLite database path |
...
# Or manually:
yousef config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.yousef/.env
```
...
# Or manually:
yousef config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$YOUSEF_HOME/byterover/` (profile-scoped)
...
# Or manually:
yousef config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.yousef/.env
```

**Config:** `$YOUSEF_HOME/supermemory.json`
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hermes-{identity}` → `hermes-coder`) to isolate memories per Sovereign profile
...
Each provider's data is isolated per [profile](/docs/mission-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$YOUSEF_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$YOUSEF_HOME/` so each profile has its own credentials
