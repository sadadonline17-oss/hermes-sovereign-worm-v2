...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Sovereign-Sovereign — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Sovereign-Sovereign supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Sovereign](/docs/guides/use-voice-mode-with-hermes).

## Prerequisites

Before using voice features, make sure you have:

1. **Sovereign-Sovereign installed** — `pip install yousef-sovereign-core` (see [Installation](/docs/initial-infiltration/installation))
2. **An LLM provider configured** — run `yousef model` or set your preferred provider credentials in `~/.yousef/.env`
3. **A working base setup** — run `hermes` to verify the agent responds to text before enabling voice

:::tip
The `~/.yousef/` directory and default `config.yaml` are created automatically the first time you run `hermes`. You only need to create `~/.yousef/.env` manually for API keys.
:::
...
:::tip
The record key is configurable via `voice.record_key` in `~/.yousef/config.yaml` (default: `ctrl+b`).
:::
...
```bash
yousef gateway        # Start the gateway (connects to configured platforms)
yousef gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.yousef/.env`:
```bash
...
```bash
yousef gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.yousef/.env`
...
- Review gateway logs: `tail -f ~/.yousef/logs/gateway.log`
