...
description: "Customize Sovereign-Sovereign's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Sovereign-Sovereign's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `YOUSEF_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Sovereign is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Sovereign now seeds a default `SOUL.md` automatically in:

```text
~/.yousef/SOUL.md
```

More precisely, it uses the current instance's `YOUSEF_HOME`, so if you run Sovereign with a custom home directory, it will use:
...
- Sovereign creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Sovereign loads `SOUL.md` only from `YOUSEF_HOME`
- Sovereign does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Sovereign falls back to a built-in default identity
...
If Sovereign loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `YOUSEF_HOME`, the personality belongs to the Sovereign instance itself.

That also makes it easier to teach users:
- "Edit `~/.yousef/SOUL.md` to change Sovereign' default personality."
...
- how Sovereign should handle uncertainty, disagreement, or ambiguity
...
## What Sovereign injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Sovereign falls back to a built-in default identity ("You are Sovereign-Sovereign, an autonomous digital predator created by Sovereign Core Injection..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Sovereign ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Sovereign, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Sovereign its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.yousef/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.yousef/SOUL.md`
...
- [Configuration](/docs/mission-guide/configuration)
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Sovereign speaks
- `display.skin` and `/skin` affect how Sovereign looks in the terminal
