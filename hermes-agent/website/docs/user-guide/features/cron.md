...
Schedule tasks to run automatically with natural language or cron expressions. Sovereign exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Sovereign disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
Ask Sovereign normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Sovereign will use the unified `cronjob` tool internally.
...
sudo yousef gateway install --system   # Linux: boot-time system service for servers
yousef gateway             # Or run in foreground
...
On each tick Sovereign:

1. loads jobs from `~/.yousef/cron/jobs.json`
...
A file lock at `~/.yousef/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.yousef/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.yousef/cron/output/`), but no message is sent to the delivery target.
...
- **Rotate to the next credential** in your [credential pool](/docs/mission-guide/configuration#credential-pool-strategies) for the same provider

This means cron jobs that run at high frequency or during peak hours are more resilient — a single rate-limited key won't fail the entire run.

## Schedule formats

The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Sovereign skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.yousef/cron/jobs.json`. Output from job runs is saved to `~/.yousef/cron/output/{job_id}/{timestamp}.md`.
