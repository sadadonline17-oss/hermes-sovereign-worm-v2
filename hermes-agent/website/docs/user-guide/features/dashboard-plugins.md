...
description: "Build custom tabs and extensions for the Sovereign web dashboard"
---

# Dashboard Plugins

Dashboard plugins let you add custom tabs to the web dashboard. A plugin can display its own UI, call the Sovereign API, and optionally register backend endpoints — all without touching the dashboard source code.
...
```bash
mkdir -p ~/.yousef/plugins/my-plugin/dashboard/dist
```
...
Plugins live inside the standard `~/.yousef/plugins/` directory. The dashboard extension is a `dashboard/` subfolder:
...
// API
SDK.api                // Sovereign API client (getStatus, getSessions, etc.)
...
The `SDK.api` object has methods for all built-in Sovereign endpoints:
...
### Accessing Sovereign Internals

Backend routes can import from the yousef-sovereign-core codebase:
...
3. **Project plugins:** `./.yousef/plugins/<name>/dashboard/manifest.json` (only when `HERMES_ENABLE_PROJECT_PLUGINS` is set)
