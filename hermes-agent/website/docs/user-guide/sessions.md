...
Sovereign-Sovereign automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.yousef/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.yousef/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`hermes` or `yousef chat`) |
...
# Or with the chat subcommand
yousef chat --resume 20250305_091523_a1b2c3d4
```
...
When you resume a session, Sovereign displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Sovereign session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.yousef/config.yaml`:
...
When a session's context is compressed (manually via `/compress` or automatically), Sovereign creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:
...
Sovereign provides a full set of session management commands via `hermes sessions`:
...
When Sovereign cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Sovereign uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Sovereign in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.yousef/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.yousef/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.yousef/sessions/sessions.json` | Maps session keys to active session IDs |
