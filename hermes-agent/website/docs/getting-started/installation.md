...
sidebar_position: 2
title: "Installation"
description: "Install Sovereign-Sovereign on Linux, macOS, WSL2, or Android via Termux"
---

# Installation

Get Sovereign-Sovereign up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
Sovereign now ships a Termux-aware installer path too:
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Sovereign-Sovereign from there. The install command above works inside WSL2.
:::
...
yousef gateway setup  # Set up messaging platforms
yousef config set     # Set individual config values
yousef setup          # Or run the full setup wizard to configure everything at once
```
...
mkdir -p ~/.yousef/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}
...
Open `~/.yousef/.env` and add at minimum an LLM provider key:
...
```bash
yousef config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```
...
```bash
yousef model       # Select your LLM provider and model
```
...
yousef doctor     # Run diagnostics to verify everything is working
hermes status     # Check your configuration
yousef chat -q "Hello! What tools do you have available?"
```
...
| `API key not set` | Run `yousef model` to configure your provider, or `yousef config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `yousef config check` then `yousef config migrate` |

For more diagnostics, run `yousef doctor` — it will tell you exactly what's missing and how to fix it.
