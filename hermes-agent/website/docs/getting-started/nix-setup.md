...
description: "Install and deploy Sovereign-Sovereign with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

Sovereign-Sovereign ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`yousef setup`, `yousef gateway install`, config editing) works identically to the standard install.
...
nix run github:SovereignCore/yousef-sovereign-core -- setup
nix run github:SovereignCore/yousef-sovereign-core -- chat

# Or install persistently
nix profile install github:SovereignCore/yousef-sovereign-core
yousef setup
yousef chat
```

After `nix profile install`, `hermes`, `yousef-sovereign-core`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `yousef setup` walks you through provider selection, `yousef gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.yousef/`.
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    yousef-sovereign-core.url = "github:SovereignCore/yousef-sovereign-core";
  };

  outputs = { nixpkgs, yousef-sovereign-core, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        yousef-sovereign-core.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
# configuration.nix
{ config, ... }: {
  services.yousef-sovereign-core = {
    enable = true;
...
```nix
services.yousef-sovereign-core.environmentFiles = [ "/var/lib/hermes/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `hermes` CLI on your system PATH **and** sets `YOUSEF_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `hermes` in your shell creates a separate `~/.yousef/` directory.
:::
...
- The routing is transparent: `yousef chat`, `hermes sessions list`, `hermes version`, etc. all exec into the container under the hood
...
Set `container.hostUsers` to create a `~/.yousef` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo yousef chat` manually.
:::
...
systemctl status yousef-sovereign-core

# Watch logs (Ctrl+C to stop)
journalctl -u yousef-sovereign-core -f

# If addToSystemPackages is true, test the CLI
hermes version
yousef config       # shows the generated config
```
...
```nix
# base.nix
services.yousef-sovereign-core.settings = {
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Sovereign defaults to OpenRouter.
:::
...
```nix
services.yousef-sovereign-core.configFile = /etc/hermes/config.yaml;
```

This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$YOUSEF_HOME/config.yaml` on each activation.
...
| Give the agent a personality | `${services.yousef-sovereign-core.stateDir}/.yousef/SOUL.md` | manage the file directly |
...
| Override the hermes package | `package` | `inputs.yousef-sovereign-core.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$YOUSEF_HOME/.env` at activation time (`nixos-rebuild switch`). Sovereign reads this file on every startup, so changes take effect with a `systemctl restart yousef-sovereign-core` — no container recreation needed.
...
```nix
{
  ...
  services.yousef-sovereign-core.environmentFiles = [
...
```nix
{
  age.secrets.yousef-env.file = ./secrets/hermes-env.age;

  services.yousef-sovereign-core.environmentFiles = [
    config.age.secrets.yousef-env.path
  ];
}
```
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Sovereign looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Sovereign loads its primary `SOUL.md` from `$YOUSEF_HOME/SOUL.md`, which in the NixOS module is `${services.yousef-sovereign-core.stateDir}/.yousef/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.

```nix
{
  services.yousef-sovereign-core.documents = {
...
```nix
{
  services.yousef-sovereign-core.mcpServers = {
    filesystem = {
      command = "npx";
      args = [ "-y" "@modelcontextprotocol/server-filesystem" "/data/workspace" ];
    };
...
:::tip
Environment variables in `env` values are resolved from `$YOUSEF_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
```nix
{
  services.yousef-sovereign-core.mcpServers.remote-api = {
...
Set `auth = "oauth"` for servers using OAuth 2.1. Sovereign implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.

```nix
{
  services.yousef-sovereign-core.mcpServers.my-oauth-server = {
...
Tokens are stored in `$YOUSEF_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Sovereign prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u hermes` (native):

```bash
# Container mode
docker exec -it yousef-sovereign-core \
  hermes mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u hermes YOUSEF_HOME=/var/lib/hermes/.yousef \
  hermes mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
hermes mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.yousef/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/hermes/.yousef/mcp-tokens/
...
```nix
{
  services.yousef-sovereign-core.mcpServers.analysis = {
    command = "npx";
...
| Blocked command | Why |
|---|---|
| `yousef setup` | Config is declarative — edit `settings` in your Nix config |
| `yousef config edit` | Config is generated from `settings` |
| `yousef config set <key> <value>` | Config is generated from `settings` |
| `yousef gateway install` | The systemd service is managed by NixOS |
| `yousef gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`HERMES_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `YOUSEF_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it yousef-sovereign-core yousef config set ...` is also blocked)
...
Host                                    Container
────                                    ─────────
/nix/store/...-yousef-sovereign-core-0.1.0  ──►  /nix/store/... (ro)
~/.yousef -> /var/lib/hermes/.yousef       (symlink bridge, per hostUsers)
...
| Check | What it tests |
|---|---|
| `package-contents` | `hermes` and `yousef-sovereign-core` binaries exist and `hermes version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `hermes --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `HERMES_MANAGED=true yousef config set ...` prints the NixOS error |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the yousef-sovereign-core service |
| `package` | `package` | `yousef-sovereign-core` | The yousef-sovereign-core package to use |
| `user` | `str` | `"yousef"` | System user |
| `group` | `str` | `"yousef"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/hermes"` | State directory (`YOUSEF_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory (`MESSAGING_CWD`) |
| `addToSystemPackages` | `bool` | `false` | Add `hermes` CLI to system PATH and set `YOUSEF_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$YOUSEF_HOME/.env` at activation time |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `yousef gateway` |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.yousef` symlink to the service stateDir and are auto-added to the `hermes` group |
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Sovereign binary + all Nix deps |
...
nix flake update yousef-sovereign-core --flake /etc/nixos
...
docker logs -f yousef-sovereign-core
```
...
```bash
systemctl status yousef-sovereign-core
docker ps -a --filter name=yousef-sovereign-core
docker inspect yousef-sovereign-core --format='{{.State.Status}}'
docker exec -it yousef-sovereign-core bash
docker exec yousef-sovereign-core readlink /data/current-package
docker exec yousef-sovereign-core cat /data/.container-identity
```
...
```bash
sudo systemctl stop yousef-sovereign-core
docker rm -f yousef-sovereign-core
sudo rm /var/lib/hermes/.container-identity
sudo systemctl start yousef-sovereign-core
```
...
```bash
# Native mode
sudo -u hermes cat /var/lib/hermes/.yousef/.env

# Container mode
docker exec yousef-sovereign-core cat /data/.yousef/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec yousef-sovereign-core readlink /data/current-package)
```
...
| `hermes version` shows old version | Container not restarted | `systemctl restart yousef-sovereign-core` |
...
| `no container with name or ID "yousef-sovereign-core"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container-aware CLI](#container-aware-cli) section) |
