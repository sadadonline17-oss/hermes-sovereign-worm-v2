...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.yousef/config.yaml."""
    try:
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        ...
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=yousef-sovereign-core"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "yousef-sovereign-core"
    return _exa_client
