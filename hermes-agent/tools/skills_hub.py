...
Used by yousef_shtiwe_cli_core/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("yousef", {})
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Sovereign Core Injection) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.yousef/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hermes_meta = meta_block.get("yousef", {})
                if isinstance(hermes_meta, dict):
                    tags = hermes_meta.get("tags", [])
...
HERMES_INDEX_URL = "https://yousef-sovereign-core.nousresearch.com/docs/api/skills-index.json"
