    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "YOUSEF SHTIWE"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses YOUSEF_HOME so each profile gets its own OAuth tokens.
    Layout: ``YOUSEF_HOME/mcp-tokens/``
    """
    try:
        from yousef_constants import get_hermes_home
        base = Path(get_hermes_home())
    except ImportError:
        base = Path(os.environ.get("YOUSEF_HOME", str(Path.home() / ".yousef")))
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        YOUSEF_HOME/mcp-tokens/<server_name>.json         -- tokens
        YOUSEF_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "YOUSEF SHTIWE")
    scope = cfg.get("scope")
