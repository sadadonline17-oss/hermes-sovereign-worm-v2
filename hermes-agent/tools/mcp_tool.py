...
discovers their tools, and registers them into the yousef-sovereign-core tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.yousef/config.yaml under the ``mcp_servers`` key.
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "YOUSEF_HOME", os.path.join(os.path.expanduser("~"), ".yousef")
                )
            )
            candidates = [
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.yousef/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.yousef/.env`` loaded at startup).
    """
    try:
