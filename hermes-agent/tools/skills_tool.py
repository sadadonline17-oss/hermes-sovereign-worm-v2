...
# All skills live in ~/.yousef/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
YOUSEF_HOME = get_hermes_home()
SKILLS_DIR = YOUSEF_HOME / "skills"
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from YOUSEF_HOME/.env."""
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.yousef/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.yousef/skills/ and external dirs.
...
def _serve_plugin_skill(
    ...
    from yousef_shtiwe_cli_core.plugins import _get_disabled_plugins, get_plugin_manager
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        if ":" in name:
            ...
            from yousef_shtiwe_cli_core.plugins import discover_plugins, get_plugin_manager
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.yousef/skills/): {skill_md}")
        ...
        # Check metadata.yousef.* first (agentskills.io convention), fall back to top-level
        hermes_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("yousef", {}) or {}
