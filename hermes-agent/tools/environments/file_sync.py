...
def iter_sync_files(container_base: str = "/root/.yousef") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.yousef to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.yousef/`` directory as a tar archive,
        ...
        # against an uninitialized remote .yousef/ directory.
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.yousef/skills/a.md`` →
        ``~/.yousef/skills/a.md``, a new remote file at
        ``/root/.yousef/skills/b.md`` maps to ``~/.yousef/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
