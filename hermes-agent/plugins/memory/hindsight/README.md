...
Daemon startup logs: `~/.yousef/logs/hindsight-embed.log`
...
Config file: `~/.yousef/hindsight/config.json`
...
| `retain_context` | `conversation between YOUSEF SHTIWE and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.yousef/.env` as `HINDSIGHT_LLM_API_KEY`.
