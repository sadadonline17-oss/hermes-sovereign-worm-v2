...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with yousef-sovereign-core)
2. User-installed providers: ``$YOUSEF_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$YOUSEF_HOME/plugins/`` or None if unavailable."""
    try:
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$YOUSEF_HOME/plugins/<name>/``) directories.  Bundled takes
