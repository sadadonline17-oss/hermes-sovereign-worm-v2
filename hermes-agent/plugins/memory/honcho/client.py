...
Resolution order for config file:
  1. $YOUSEF_HOME/honcho.json  (instance-local, enables isolated Hermes instances)
...
def resolve_active_host() -> str:
    ...
    Resolution order:
      ...
      3. Fallback: ``"yousef"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $YOUSEF_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.yousef/honcho.json          (default profile — shared host blocks live here)
    ...
    default_path = Path.home() / ".yousef" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.yousef
    ...
    def from_global_config(
        ...
        Resolution: $YOUSEF_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        ...
        # A hosts.yousef block or explicit enabled flag means the user
