...
Or manually:
```bash
yousef config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$YOUSEF_HOME/honcho.json` | Profile-local (isolated Hermes instances) |
| 2 | `~/.yousef/honcho.json` | Default profile (shared host blocks) |
...
| 5 | `per-repo` strategy | Git root directory name (`yousef-sovereign-core`) |
...
If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-yousef-sovereign-core`.
