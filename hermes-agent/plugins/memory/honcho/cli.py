...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "yousef" else ""
...
def cmd_sync(args) -> None:
    ...
    """
    try:
        from yousef_shtiwe_cli_core.profiles import list_profiles
...
def _local_config_path() -> Path:
    ...
    Always returns $YOUSEF_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    current_ai = hermes_host.get("aiPeer") or cfg.get("aiPeer", "yousef")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        hermes_host["aiPeer"] = new_ai

    current_workspace = hermes_host.get("workspace") or cfg.get("workspace", "yousef")
    ...
    try:
        ...
        print("  Run: yousef config set memory.provider honcho")
...
def cmd_migrate(args) -> None:
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'yousef'")
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from yousef_shtiwe_cli_core.memory_setup import cmd_setup_provider
