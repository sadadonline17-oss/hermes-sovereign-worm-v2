...
This directory contains the integration layer between **yousef-sovereign-core's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  YousefShtiweBaseEnv    │  hermes_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
...
**YousefShtiweBaseEnv** (`hermes_base_env.py`) extends BaseEnv with yousef-sovereign-core specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves yousef-sovereign-core toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `YousefShtiweBaseEnv` and implement:
...
`YousefShtiweLoop` is the reusable multi-turn agent engine. It runs the same pattern as yousef-sovereign-core's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** yousef-sovereign-core tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any yousef-sovereign-core tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some yousef-sovereign-core tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
├── hermes_base_env.py            # Abstract base (YousefShtiweBaseEnv)
├── agent_loop.py                 # Multi-turn agent engine (YousefShtiweLoop)
...
2. Create your env file inheriting from `YousefShtiweBaseEnv`
...
class MyEnvConfig(YousefShtiweEnvConfig):
    pass  # Add custom fields as needed

class MyEnv(YousefShtiweBaseEnv):
    name = "my-env"
...
2. Inherit from `YousefShtiweBaseEnv`
