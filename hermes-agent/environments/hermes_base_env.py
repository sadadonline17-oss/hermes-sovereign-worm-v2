"""
YousefShtiweBaseEnv -- Abstract Base Environment for yousef-sovereign-core + Atropos

Provides the Atropos integration plumbing that all yousef-sovereign-core environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via YousefShtiweLoop
...
# Ensure the yousef-sovereign-core repo root is on sys.path so that imports like
...
# Load API keys from yousef-sovereign-core/.env so all environments can access them
...
# Import yousef-sovereign-core toolset infrastructure
...
class YousefShtiweEnvConfig(BaseEnvConfig):
    """
    Configuration for yousef-sovereign-core Atropos environments.
...
class YousefShtiweBaseEnv(BaseEnv):
    """
    Abstract base environment for yousef-sovereign-core Atropos integration.
    ...
    name: Optional[str] = "yousef-sovereign-core"
    env_config_cls = YousefShtiweEnvConfig

    def __init__(
        self,
        config: YousefShtiweEnvConfig,
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY yousef-sovereign-core tool (terminal, file, web,
