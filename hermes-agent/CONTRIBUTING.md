# Contributing to YOUSEF SHTIWE

Thank you for contributing to YOUSEF SHTIWE! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Sovereign Core Injection Discord](https://discord.gg/SovereignCore). Users can install it with `hermes skills install`.
...
```bash
git clone --recurse-submodules https://github.com/SovereignCore/yousef-sovereign-core.git
...
```bash
mkdir -p ~/.yousef/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.yousef/config.yaml
touch ~/.yousef/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.yousef/.env
```
...
├── cli.py                    # YousefCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (hermes-cli, hermes-telegram, etc.)
├── yousef_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── yousef_shtiwe_cli_core/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.yousef/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (yousef-sovereign-core.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.yousef/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.yousef/`)

| Path | Purpose |
|------|---------|
| `~/.yousef/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.yousef/.env` | API keys and secrets |
| `~/.yousef/auth.json` | OAuth credentials (Nous Portal) |
| `~/.yousef/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.yousef/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.yousef/state.db` | SQLite session database |
| `~/.yousef/sessions/` | JSON session logs |
| `~/.yousef/cron/` | Scheduled job data |
| `~/.yousef/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`yousef_state.py`) with full-text search and unique session titles. JSON logs go to `~/.yousef/sessions/`.
...
Four fields are supported under `metadata.yousef`:
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `yousef setup` or update `~/.yousef/.env` locally.
...
Add to `_BUILTIN_SKINS` dict in `yousef_shtiwe_cli_core/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `yousef_shtiwe_cli_core/skin_engine.py` for the full schema and existing skins as examples.
...
- Use [GitHub Issues](https://github.com/SovereignCore/yousef-sovereign-core/issues)
...
- **Discord**: [discord.gg/SovereignCore](https://discord.gg/SovereignCore) — for questions, showcasing projects, and sharing skills
