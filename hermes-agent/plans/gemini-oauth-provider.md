...
## Client ID
- Need to register a "Desktop app" OAuth client on a Sovereign Core Injection GCP project
...
## Token Lifecycle
- Store at `~/.yousef/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `yousef_shtiwe_cli_core/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `yousef_shtiwe_cli_core/models.py` — Add Gemini model catalog
4. `yousef_shtiwe_cli_core/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `yousef_shtiwe_cli_core/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `yousef_shtiwe_cli_core/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Sovereign Core Injection GCP project with Desktop OAuth client registered
...
- yousef-sovereign-core Copilot OAuth: `yousef_shtiwe_cli_core/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
