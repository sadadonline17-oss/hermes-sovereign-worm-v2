# OpenAI-Compatible API Server for Hermes-Sovereign
...
SSE streaming. By exposing this endpoint, yousef-sovereign-core becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  yousef-sovereign-core   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.yousef/.env`
2. Run `yousef gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with yousef-sovereign-core through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns yousef-sovereign-core as a model) |
...
```json
{
  "object": "list",
  "data": [{
    "id": "yousef-sovereign-core",
    "object": "model",
    "created": 1710000000,
    "owned_by": "yousef-sovereign-core"
  }]
}
```
...
But yousef-sovereign-core sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "yousef-sovereign-core"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured yousef-sovereign-core model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of yousef-sovereign-core's core prompt
...
Once implemented, yousef-sovereign-core works as a drop-in backend for:
