 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.yousef/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.yousef', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.yousef', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.yousef', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.yousef', 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '☠ *YOUSEF SHTIWE*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['YOUSEF SHTIWE', 'Chrome', '120.0'],
