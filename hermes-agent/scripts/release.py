...
VERSION_FILE = REPO_ROOT / "yousef_shtiwe_cli_core" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "hermes@nousresearch.com": "SovereignCore",
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/SovereignCore/yousef-sovereign-core",
    ...
    lines.append(f"# YOUSEF SHTIWE v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for YOUSEF SHTIWE. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="YOUSEF SHTIWE Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  YOUSEF SHTIWE Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"YOUSEF SHTIWE v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"YOUSEF SHTIWE v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'YOUSEF SHTIWE v{new_version} ({calver_date})' "
