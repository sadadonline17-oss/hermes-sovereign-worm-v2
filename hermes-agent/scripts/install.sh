...
# Configuration
REPO_URL_SSH="git@github.com:SovereignCore/yousef-sovereign-core.git"
REPO_URL_HTTPS="https://github.com/SovereignCore/yousef-sovereign-core.git"
YOUSEF_HOME="${YOUSEF_HOME:-$HOME/.yousef}"
INSTALL_DIR="${HERMES_INSTALL_DIR:-$YOUSEF_HOME/yousef-sovereign-core}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.yousef/yousef-sovereign-core)"
            echo "  --hermes-home PATH  Data directory (default: ~/.yousef, or \$YOUSEF_HOME)"
...
print_banner() {
    ...
    echo "│             ☠ YOUSEF SHTIWE Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Sovereign Core.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/SovereignCore/yousef-sovereign-core/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$YOUSEF_HOME/node/bin/node" ]; then
        export PATH="$YOUSEF_HOME/node/bin:$PATH"
        local found_ver=$("$YOUSEF_HOME/node/bin/node" --version)
...
install_node() {
    ...
    log_info "Extracting to ~/.yousef/node/..."
    ...
    # Place into ~/.yousef/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$YOUSEF_HOME/node"
    mkdir -p "$YOUSEF_HOME"
    mv "$extracted_dir" "$YOUSEF_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$YOUSEF_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$YOUSEF_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$YOUSEF_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$YOUSEF_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$YOUSEF_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.yousef/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "YOUSEF SHTIWE itself does not require or retain root access."
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# YOUSEF SHTIWE — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# YOUSEF SHTIWE — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    # Create ~/.yousef directory structure (config at top level, code in subdir)
    mkdir -p "$YOUSEF_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.yousef/.env (top level, easy to find)
    if [ ! -f "$YOUSEF_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$YOUSEF_HOME/.env"
            log_success "Created ~/.yousef/.env from template"
        else
            touch "$YOUSEF_HOME/.env"
            log_success "Created ~/.yousef/.env"
        fi
    else
        log_info "~/.yousef/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.yousef/config.yaml (top level, easy to find)
    if [ ! -f "$YOUSEF_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$YOUSEF_HOME/config.yaml"
            log_success "Created ~/.yousef/config.yaml from template"
        fi
    else
        log_info "~/.yousef/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$YOUSEF_HOME/SOUL.md" ]; then
        cat > "$YOUSEF_HOME/SOUL.md" << 'SOUL_EOF'
# YOUSEF SHTIWE Persona
...
-->
SOUL_EOF
        log_success "Created ~/.yousef/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.yousef/"

    # Seed bundled skills into ~/.yousef/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.yousef/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.yousef/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$YOUSEF_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$YOUSEF_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.yousef/skills/"
        fi
    fi
}
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'yousef setup' after install."
        return 0
    fi
    ...
    # Run yousef setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m yousef_shtiwe_cli_core.main setup < /dev/tty
    else
        python -m yousef_shtiwe_cli_core.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$YOUSEF_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    WHATSAPP_SESSION="$YOUSEF_HOME/whatsapp/session/creds.json"
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'yousef gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: yousef gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: yousef gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $HERMES_CMD gateway > "$YOUSEF_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.yousef/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: yousef gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: yousef gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.yousef/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.yousef/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.yousef/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.yousef/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.yousef/yousef-sovereign-core/"
    echo ""
    ...
    echo -e "   ${GREEN}yousef setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}yousef config${NC}       View/edit configuration"
    echo -e "   ${GREEN}yousef config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}yousef gateway install${NC} Install gateway service (messaging + cron)"
    ...
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'yousef' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'yousef' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
