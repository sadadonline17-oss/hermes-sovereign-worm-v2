# ============================================================================
# YOUSEF SHTIWE Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/SovereignCore/yousef-sovereign-core/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [string]$InstallDir = "$env:LOCALAPPDATA\hermes\yousef-sovereign-core"
)
...
$RepoUrlSsh = "git@github.com:SovereignCore/yousef-sovereign-core.git"
$RepoUrlHttps = "https://github.com/SovereignCore/yousef-sovereign-core.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ☠ YOUSEF SHTIWE Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
    Write-Host "│  An open source AI agent by Sovereign Core.              │" -ForegroundColor Magenta
    Write-Host "└─────────────────────────────────────────────────────────┘" -ForegroundColor Magenta
    Write-Host ""
}
...
function Test-Node {
    ...
    # Fallback: download binary zip to ~/.yousef/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                ...
                $version = & "$HermesHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.yousef/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/SovereignCore/yousef-sovereign-core/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\yousef-sovereign-core-$Branch.zip"
                $extractPath = "$env:TEMP\yousef-sovereign-core-extract"
...
function Set-PathVariable {
    ...
    # Set YOUSEF_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\hermes instead
    # of the Unix default ~/.yousef
    $currentHermesHome = [Environment]::GetEnvironmentVariable("YOUSEF_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("YOUSEF_HOME", $HermesHome, "User")
        Write-Success "Set YOUSEF_HOME=$HermesHome"
    }
    $env:YOUSEF_HOME = $HermesHome
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.yousef directory structure
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.yousef/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.yousef/.env"
        }
    } else {
        Write-Info "~/.yousef/.env already exists, keeping it"
    }
    ...
    if (-not (Test-Path $configPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.yousef/config.yaml from template"
        }
    } else {
        Write-Info "~/.yousef/config.yaml already exists, keeping it"
    }
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.yousef/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.yousef/"
    
    # Seed bundled skills into ~/.yousef/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.yousef/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.yousef/skills/"
        } catch {
            ...
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                ...
                Write-Success "Skills copied to ~/.yousef/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m yousef_shtiwe_cli_core.main setup
    } else {
...
function Start-GatewayIfConfigured {
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: yousef gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: yousef gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$HermesHome\yousef-sovereign-core\"
    Write-Host ""
    ...
    Write-Host "   yousef setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   yousef config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   yousef config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   yousef gateway      " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/SovereignCore/yousef-sovereign-core/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
