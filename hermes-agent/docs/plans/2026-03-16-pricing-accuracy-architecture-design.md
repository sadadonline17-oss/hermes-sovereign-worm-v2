...
Sovereign should only show dollar costs when they are backed by an official source for the user's actual billing path.
...
Current Sovereign behavior has four structural issues:
...
Sovereign must stop keying pricing solely by `model`.
...
Live requests may produce only an estimate initially. Sovereign should reconcile them later when a provider exposes actual billed cost.

Suggested flow:

1. Agent call completes.
2. Sovereign stores canonical usage plus reconciliation ids.
3. Sovereign computes an immediate estimate if a pricing source exists.
...
That path cannot satisfy the product requirement. Sovereign should instead migrate to:
...
This is the minimum architecture that makes the statement "Sovereign pricing is backed by official sources where possible, and otherwise clearly labeled" defensible.
