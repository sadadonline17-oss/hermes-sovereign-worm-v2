...
**PR:** SovereignCore/yousef-sovereign-core#7543
...
### Changes to `yousef_shtiwe_cli_core/main.py`
...
### Changes to `yousef_shtiwe_cli_core/config.py`
...
Note: there is a theoretical race between the `[ -d ... ]` check and the `mv` (something could create/remove the directory in between). In practice this is a NixOS activation script running as root during `nixos-rebuild switch` — no other process should be touching `~/.yousef` at that moment. Not worth adding locking for.
...
The existing test file (`tests/yousef_shtiwe_cli_core/test_container_aware_cli.py`) has 16 tests. With the simplified exec model, several are obsolete.
