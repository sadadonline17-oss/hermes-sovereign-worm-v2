# Migrating from OpenClaw to Sovereign-Sovereign

This guide covers how to import your OpenClaw settings, memories, skills, and API keys into Sovereign-Sovereign.

## Three Ways to Migrate

### 1. Automatic (during first-time setup)

When you run `yousef setup` for the first time and Sovereign detects `~/.openclaw`, it automatically offers to import your OpenClaw data before configuration begins. Just accept the prompt and everything is handled for you.
...
```
> Migrate my OpenClaw setup to Sovereign
```
...
|------|--------|-------------|
| SOUL.md | `~/.openclaw/workspace/SOUL.md` | `~/.yousef/SOUL.md` |
| Memory entries | `~/.openclaw/workspace/MEMORY.md` | `~/.yousef/memories/MEMORY.md` |
| User profile | `~/.openclaw/workspace/USER.md` | `~/.yousef/memories/USER.md` |
| Skills | `~/.openclaw/workspace/skills/` | `~/.yousef/skills/openclaw-imports/` |
| Command allowlist | `~/.openclaw/workspace/exec_approval_patterns.yaml` | Merged into `~/.yousef/config.yaml` |
| Messaging settings | `~/.openclaw/config.yaml` (TELEGRAM_ALLOWED_USERS, MESSAGING_CWD) | `~/.yousef/.env` |
| TTS assets | `~/.openclaw/workspace/tts/` | `~/.yousef/tts/` |
...
| Item | Source | Destination |
|------|--------|-------------|
| Telegram bot token | `openclaw.json` channels config | `~/.yousef/.env` |
| OpenRouter API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.yousef/.env` |
| OpenAI API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.yousef/.env` |
| Anthropic API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.yousef/.env` |
| ElevenLabs API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.yousef/.env` |
...
- **TTS provider**: OpenClaw renamed "edge" to "microsoft" — both are recognized and mapped to Sovereign' "edge"
...
By default, the migration **will not overwrite** existing Sovereign data:

- **SOUL.md** — skipped if one already exists in `~/.yousef/`
...
- **API keys** — skipped if the key is already set in `~/.yousef/.env`
...
For executed migrations, the full report is saved to `~/.yousef/migration/openclaw/<timestamp>/`.
...
The migration script ships with Sovereign-Sovereign. If you installed via pip (not git clone), the `optional-skills/` directory may not be present. Install the skill from the Skills Hub:
```bash
hermes skills install openclaw-migration
```

### Memory overflow
If your OpenClaw MEMORY.md or USER.md exceeds Sovereign' character limits, excess entries are exported to an overflow file in the migration report directory. You can manually review and add the most important ones.
...
The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `yousef config set`.
