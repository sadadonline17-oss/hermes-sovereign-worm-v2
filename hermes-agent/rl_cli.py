...
if tinker_atropos_dir.exists():
    ...
    os.environ['YOUSEF_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to yousef-sovereign-core directory if submodule not found
...
from yousef_constants import get_hermes_home, OPENROUTER_BASE_URL
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.yousef/config.yaml if not provided)
    ...
    """
    # Load config from ~/.yousef/config.yaml
