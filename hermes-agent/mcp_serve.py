...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using YOUSEF_HOME."""
    try:
        from yousef_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("YOUSEF_HOME", Path.home() / ".yousef")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("YOUSEF_HOME", Path.home() / ".yousef")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("YOUSEF_HOME", Path.home() / ".yousef")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    ...
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            ...
            "Install with: pip install 'yousef-sovereign-core[mcp]'"
        )

    mcp = FastMCP(
        "yousef",
        instructions=(
            "YOUSEF SHTIWE messaging bridge. Use these tools to interact with "
