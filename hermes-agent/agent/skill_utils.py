...
from yousef_constants import get_config_path, get_skills_dir
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.yousef/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.yousef/skills/`` first, then external.
