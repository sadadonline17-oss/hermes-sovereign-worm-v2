"""Anthropic Messages API adapter for YOUSEF SHTIWE.
...
# Stores credentials in ~/.yousef/.anthropic_oauth.json (our own file).
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Hermes-managed OAuth credentials from ~/.yousef/.anthropic_oauth.json."""
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("YOUSEF SHTIWE", "Claude Code")
                text = text.replace("YOUSEF SHTIWE", "Claude Code")
                text = text.replace("yousef-sovereign-core", "claude-code")
                text = text.replace("Sovereign Core Injection", "Anthropic")
                block["text"] = text
