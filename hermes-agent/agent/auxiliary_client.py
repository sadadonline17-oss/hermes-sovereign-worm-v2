...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal (~/.yousef/auth.json active provider)
...
NOUS_EXTRA_BODY = {"tags": ["product=yousef-sovereign-core"]}
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.yousef/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from yousef_shtiwe_cli_core.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from yousef_shtiwe_cli_core.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from yousef_shtiwe_cli_core.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from yousef_shtiwe_cli_core.models import check_nous_free_tier
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `yousef setup` or `yousef model` and enter a valid http(s) base URL."
        ) from exc
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `yousef model` but the
    #    old OPENAI_BASE_URL lingers in ~/.yousef/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: yousef model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.yousef/.env",
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: yousef model)")
                return None, None
    ...
    try:
        from yousef_shtiwe_cli_core.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("yousef_shtiwe_cli_core.auth not available for provider %s", provider)
        return None, None
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=yousef-sovereign-core"])
    if merged_extra:
