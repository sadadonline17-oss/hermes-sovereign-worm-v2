"""Centralized logging setup for YOUSEF SHTIWE.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.yousef/logs/`` (profile-aware via ``get_hermes_home()``).
...
from yousef_constants import get_config_path, get_hermes_home
...
COMPONENT_PREFIXES = {
    ...
    "tools": ("tools",),
    "cli": ("yousef_shtiwe_cli_core", "cli"),
    "cron": ("cron",),
}
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from yousef_shtiwe_cli_core.config import is_managed
