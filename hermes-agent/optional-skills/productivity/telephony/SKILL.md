...
- save provider credentials into `~/.yousef/.env`
...
```bash
SCRIPT="$(find ~/.yousef/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `~/.yousef/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.yousef/.env`
