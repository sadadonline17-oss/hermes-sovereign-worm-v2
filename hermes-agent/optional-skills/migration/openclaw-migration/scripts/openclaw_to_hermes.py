...
into YOUSEF SHTIWE, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.yousef/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.yousef/tts/.",
    },
    "discord-settings": {
...
class Migrator:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in ("file", "exec"):
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: yousef config set",
                        )
                    continue
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'yousef gateway' to configure.")
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the YOUSEF SHTIWE discovers these",
            ...
            "- Run `yousef setup` to configure any remaining settings",
        ...
        notes.extend([
            "- Run `yousef gateway install` if you need the gateway service",
            "- Review `~/.yousef/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into YOUSEF SHTIWE.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=str(Path.home() / ".yousef"), help="Hermes home directory")
    parser.add_argument(
        "--workspace-target",
...
def main() -> int:
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.yousef/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.yousef/config.yaml")
