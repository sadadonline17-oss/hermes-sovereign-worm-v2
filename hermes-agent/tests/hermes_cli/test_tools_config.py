"""Tests for yousef_shtiwe_cli_core.tools_config platform tool persistence."""

from unittest.mock import patch

from yousef_shtiwe_cli_core.tools_config import (
    _configure_provider,
    _get_platform_tools,
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/SovereignCore/yousef-sovereign-core/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("yousef_shtiwe_cli_core.tools_config.save_config"):
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    monkeypatch.setattr("yousef_shtiwe_cli_core.tools_config.managed_nous_tools_enabled", lambda: True)
    config = {"model": {"provider": "nous"}}

    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.nous_subscription.get_nous_auth_status",
...
def test_visible_providers_hide_nous_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.setattr("yousef_shtiwe_cli_core.tools_config.managed_nous_tools_enabled", lambda: False)
    config = {"model": {"provider": "nous"}}
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("yousef_shtiwe_cli_core.tools_config._run_post_setup", lambda key: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web", "image_gen", "tts", "browser"},
    )
    monkeypatch.setattr("yousef_shtiwe_cli_core.tools_config.save_config", lambda config: None)
    ...
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.nous_subscription.get_nous_auth_status",
    ...
    configured = []
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from yousef_shtiwe_cli_core.tools_config import PLATFORMS
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from yousef_shtiwe_cli_core.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from yousef_shtiwe_cli_core.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/SovereignCore/yousef-sovereign-core/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from yousef_shtiwe_cli_core.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from yousef_shtiwe_cli_core.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from yousef_shtiwe_cli_core.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("yousef_shtiwe_cli_core.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("yousef_shtiwe_cli_core.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("yousef_shtiwe_cli_core.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
