...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `yousef_shtiwe_cli_core.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "yousef"
    called = []

    fake_main_module = types.ModuleType("yousef_shtiwe_cli_core.main")

    def fake_main():
        called.append("yousef_shtiwe_cli_core.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "yousef_shtiwe_cli_core.main", fake_main_module)
    ...
    assert called == ["yousef_shtiwe_cli_core.main"]
