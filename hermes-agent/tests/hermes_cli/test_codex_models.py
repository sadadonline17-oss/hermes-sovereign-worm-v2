...
from yousef_shtiwe_cli_core.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in yousef_shtiwe_cli_core/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from yousef_shtiwe_cli_core.codex_models import get_codex_model_ids as setup_import
...
def test_get_codex_model_ids_adds_forward_compat_models_from_templates(monkeypatch):
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.codex_models._fetch_models_from_api",
...
def test_model_command_uses_runtime_access_token_for_codex_list(monkeypatch):
    from yousef_shtiwe_cli_core.main import _model_flow_openai_codex

    captured = {}

    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.auth.get_codex_auth_status",
        lambda: {"logged_in": True},
    )
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.auth.resolve_codex_runtime_credentials",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a YousefCLI with minimal mocking."""
    ...
    with (
        ...
        cli = YousefCLI(model=model, **kwargs)
    return cli
