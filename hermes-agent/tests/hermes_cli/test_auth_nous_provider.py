...
from yousef_shtiwe_cli_core.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from yousef_shtiwe_cli_core.auth import _resolve_verify
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from yousef_shtiwe_cli_core.auth import get_nous_auth_status
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("yousef_shtiwe_cli_core.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("yousef_shtiwe_cli_core.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `yousef model` → Nous Portal → Skip (keep current) after
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import yousef_shtiwe_cli_core.auth as auth_mod
        import yousef_shtiwe_cli_core.models as models_mod
        import yousef_shtiwe_cli_core.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from yousef_shtiwe_cli_core.auth import PROVIDER_REGISTRY, _login_nous
