"""Tests that provider selection via `yousef model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated YOUSEF_HOME with a minimal string-format config."""
    ...
    monkeypatch.delenv("YOUSEF_SHTIWE_MODEL", raising=False)
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from yousef_shtiwe_cli_core.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from yousef_shtiwe_cli_core.main import _model_flow_api_key_provider
        ...
        with patch("yousef_shtiwe_cli_core.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("yousef_shtiwe_cli_core.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from yousef_shtiwe_cli_core.main import _model_flow_copilot
        from yousef_shtiwe_cli_core.config import load_config

        with patch(
            "yousef_shtiwe_cli_core.auth.resolve_api_key_provider_credentials",
            return_value={
                "provider": "copilot",
            ...
            "yousef_shtiwe_cli_core.auth._prompt_model_selection",
            return_value="gpt-5.4",
        ), patch(
            "yousef_shtiwe_cli_core.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "yousef_shtiwe_cli_core.auth.deactivate_provider",
        ):
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from yousef_shtiwe_cli_core.main import _model_flow_copilot_acp
        from yousef_shtiwe_cli_core.config import load_config

        with patch(
            "yousef_shtiwe_cli_core.auth.get_external_process_provider_status",
            return_value={
            ...
            "yousef_shtiwe_cli_core.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("yousef_shtiwe_cli_core.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("yousef_shtiwe_cli_core.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("yousef_shtiwe_cli_core.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("yousef_shtiwe_cli_core.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("yousef_shtiwe_cli_core.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("yousef_shtiwe_cli_core.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        from yousef_shtiwe_cli_core.main import _model_flow_api_key_provider
        from yousef_shtiwe_cli_core.config import load_config, get_env_value

        # User types a shell command instead of a URL at the base URL prompt
        with patch("yousef_shtiwe_cli_core.auth._prompt_model_selection", return_value="glm-5"), \
             patch("yousef_shtiwe_cli_core.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.yousef/.env"):
