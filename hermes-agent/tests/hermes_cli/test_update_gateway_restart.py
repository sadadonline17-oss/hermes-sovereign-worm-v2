...
from yousef_shtiwe_cli_core.main import cmd_update
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>YOUSEF_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: yousef gateway run'."""
        ...
        assert "Restart manually: yousef gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: yousef gateway run" in captured
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.yousef.gateway")
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.yousef/yousef-sovereign-core/venv/bin/python -m yousef_shtiwe_cli_core.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.yousef/yousef-sovereign-core/venv/bin/python -m yousef_shtiwe_cli_core.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
...
class TestGatewayModeWritesExitCodeEarly:
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point YOUSEF_HOME at a temp dir so the marker file lands there
        hermes_home = tmp_path / ".yousef"
        hermes_home.mkdir()
        monkeypatch.setenv("YOUSEF_HOME", str(hermes_home))
        import yousef_shtiwe_cli_core.config as _cfg
