...
from yousef_shtiwe_cli_core import config as hermes_config
...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M yousef_shtiwe_cli_core/main.py\n?? notes.txt\n", returncode=0)
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="yousef_shtiwe_cli_core/main.py\n", stderr="", returncode=0)
    ...
    assert "yousef_shtiwe_cli_core/main.py" in out
