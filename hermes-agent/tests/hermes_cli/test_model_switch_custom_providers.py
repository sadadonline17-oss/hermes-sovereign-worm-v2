...
The terminal `yousef model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import yousef_shtiwe_cli_core.providers as providers_mod
from yousef_shtiwe_cli_core.model_switch import list_authenticated_providers, switch_model
from yousef_shtiwe_cli_core.providers import resolve_provider_full
...
def test_switch_model_accepts_explicit_named_custom_provider(monkeypatch):
    ...
    monkeypatch.setattr("yousef_shtiwe_cli_core.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("yousef_shtiwe_cli_core.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("yousef_shtiwe_cli_core.model_switch.get_model_capabilities", lambda *a, **k: None)
