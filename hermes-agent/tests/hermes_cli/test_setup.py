...
from yousef_shtiwe_cli_core import setup as setup_mod
from yousef_shtiwe_cli_core.setup import setup_model_provider
...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("yousef_shtiwe_cli_core.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("yousef_shtiwe_cli_core.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("yousef_shtiwe_cli_core.main.select_provider_and_model", fake_select)
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import yousef_shtiwe_cli_core.gateway as gateway_mod
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("yousef_shtiwe_cli_core.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("yousef_shtiwe_cli_core.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.main._model_flow_named_custom",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("yousef_shtiwe_cli_core.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("YOUSEF_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("yousef_shtiwe_cli_core.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("yousef_shtiwe_cli_core.setup.prompt", fake_prompt)
    monkeypatch.setattr("yousef_shtiwe_cli_core.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.setup.get_nous_subscription_features",
    ...
    from yousef_shtiwe_cli_core.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("yousef_shtiwe_cli_core.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("yousef_shtiwe_cli_core.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("yousef_shtiwe_cli_core.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
...
def test_resolve_hermes_chat_argv_prefers_which(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod.shutil, "which", lambda name: "/usr/local/bin/hermes" if name == "yousef" else None)
...
def test_resolve_hermes_chat_argv_falls_back_to_module(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod.importlib.util, "find_spec", lambda name: object() if name == "yousef_shtiwe_cli_core" else None)

    assert setup_mod._resolve_hermes_chat_argv() == [sys.executable, "-m", "yousef_shtiwe_cli_core.main", "chat"]
...
def test_offer_launch_chat_manual_fallback_when_unresolvable(monkeypatch, capsys):
    ...
    assert "Run 'yousef chat' manually" in captured.out
