"""Tests for yousef_shtiwe_cli_core configuration management."""
...
class TestGetHermesHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert home == Path.home() / ".yousef"

    def test_env_override(self):
        with patch.dict(os.environ, {"YOUSEF_HOME": "/custom/path"}):
            home = get_hermes_home()
...
class TestEnsureHermesHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"YOUSEF_HOME": str(tmp_path)}):
            ensure_hermes_home()
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"YOUSEF_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"YOUSEF_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"YOUSEF_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"YOUSEF_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"YOUSEF_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_api_key_registered(self):
        ...
        from yousef_shtiwe_cli_core.config import OPTIONAL_ENV_VARS
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from yousef_shtiwe_cli_core.config import ENV_VARS_BY_VERSION
        all_vars = []
