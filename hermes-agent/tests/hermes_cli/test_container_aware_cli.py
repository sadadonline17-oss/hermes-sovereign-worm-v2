...
def container_env(tmp_path, monkeypatch):
    """Set up a fake YOUSEF_HOME with .container-mode file."""
    ...
    container_mode.write_text(
        ...
        "backend=podman\n"
        "container_name=yousef-sovereign-core\n"
        "exec_user=hermes\n"
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    assert info["container_name"] == "yousef-sovereign-core"
    assert info["exec_user"] == "yousef"
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        hermes_home = Path(tmpdir) / ".yousef"
        hermes_home.mkdir()
        ...
        with patch("hermes_constants.is_container", return_value=False), \
             patch("yousef_shtiwe_cli_core.config.get_hermes_home", return_value=hermes_home), \
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from yousef_shtiwe_cli_core.main import _exec_in_container
