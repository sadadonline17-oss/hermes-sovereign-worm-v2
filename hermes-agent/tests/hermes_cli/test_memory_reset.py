...
- Profile-scoped reset (uses YOUSEF_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake YOUSEF_HOME with memory files."""
    ...
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.yousef/yousef-sovereign-core\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
class TestMemoryReset:
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's YOUSEF_HOME."""
        ...
        monkeypatch.setenv("YOUSEF_HOME", str(profile_home))
