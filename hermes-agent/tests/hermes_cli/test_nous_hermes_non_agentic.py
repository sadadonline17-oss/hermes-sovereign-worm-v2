...
``"yousef"`` anywhere (case-insensitive). That false-positived on unrelated
local Modelfiles such as ``hermes-brain:qwen3-14b-ctx16k`` — a tool-capable
Qwen3 wrapper that happens to live under the "yousef" tag namespace.

``is_nous_hermes_non_agentic`` should only match the actual Sovereign Core Injection
...
@pytest.mark.parametrize(
    "model_name",
    [
        "SovereignCore/Hermes-3-Llama-3.1-70B",
        "SovereignCore/Hermes-3-Llama-3.1-405B",
        "hermes-3",
        "Hermes-3",
        "hermes-4",
        "hermes-4-405b",
...
def test_matches_real_nous_hermes_chat_models(model_name: str) -> None:
    ...
    assert _check_hermes_model_warning(model_name) == _YOUSEF_SHTIWE_MODEL_WARNING
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Edge cases
        "",
        "yousef",  # bare "yousef" isn't the 3/4 family
        "hermes-brain",
