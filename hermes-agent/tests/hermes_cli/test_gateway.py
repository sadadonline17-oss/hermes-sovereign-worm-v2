"""Tests for yousef_shtiwe_cli_core.gateway."""
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    ...
    assert "yousef gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo yousef gateway install --system --run-as-user alice" in out
    assert "sudo yousef gateway start --system" in out
