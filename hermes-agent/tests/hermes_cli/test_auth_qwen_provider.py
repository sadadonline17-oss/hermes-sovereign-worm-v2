"""Tests for Qwen OAuth provider authentication (yousef_shtiwe_cli_core/auth.py).
...
def qwen_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.auth._qwen_cli_auth_path", lambda: creds_path
    )
    return tmp_path
...
def test_refresh_qwen_cli_tokens_success(qwen_env):
    ...
    with patch("yousef_shtiwe_cli_core.auth.httpx") as mock_httpx:
...
def test_resolve_qwen_runtime_credentials_triggers_refresh(qwen_env):
    ...
    with patch(
        "yousef_shtiwe_cli_core.auth._refresh_qwen_cli_tokens", return_value=refreshed
    ) as mock_refresh:
