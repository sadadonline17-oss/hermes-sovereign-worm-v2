"""Tests for yousef_shtiwe_cli_core/webhook.py — webhook subscription CLI."""
...
from yousef_shtiwe_cli_core.webhook import (
    webhook_command,
    _load_subscriptions,
    _save_subscriptions,
    _subscriptions_path,
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "yousef_shtiwe_cli_core.webhook._is_webhook_enabled", lambda: True
    )
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("yousef_shtiwe_cli_core.webhook._is_webhook_enabled", lambda: False)
        webhook_command(_make_args(webhook_action="subscribe", name="blocked"))
        out = capsys.readouterr().out
        assert "not enabled" in out.lower()
        assert "yousef gateway setup" in out
        assert _load_subscriptions() == {}
    ...
    def test_real_check_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "yousef_shtiwe_cli_core.webhook._get_webhook_config",
            lambda: {},
        )
        monkeypatch.setattr(
            "yousef_shtiwe_cli_core.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import yousef_shtiwe_cli_core.webhook as wh_mod
