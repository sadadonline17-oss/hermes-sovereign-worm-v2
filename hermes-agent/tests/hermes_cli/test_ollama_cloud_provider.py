...
from yousef_shtiwe_cli_core.auth import PROVIDER_REGISTRY, resolve_provider, resolve_api_key_provider_credentials
from yousef_shtiwe_cli_core.models import _PROVIDER_MODELS, _PROVIDER_LABELS, _PROVIDER_ALIASES, normalize_provider
...
class TestOllamaCloudModelCatalog:
    ...
    def test_provider_model_ids_returns_dynamic_models(self, tmp_path, monkeypatch):
        ...
        with patch("yousef_shtiwe_cli_core.models.fetch_api_models", return_value=["qwen3.5:397b"]), \
...
class TestOllamaCloudModelPicker:
    def test_ollama_cloud_shows_model_count(self, tmp_path, monkeypatch):
        ...
        from yousef_shtiwe_cli_core.model_switch import list_authenticated_providers
...
class TestOllamaCloudMergedDiscovery:
    def test_merges_live_and_models_dev(self, tmp_path, monkeypatch):
        ...
        with patch("yousef_shtiwe_cli_core.models.fetch_api_models", return_value=["qwen3.5:397b", "glm-5"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
    ...
    def test_uses_disk_cache(self, tmp_path, monkeypatch):
        ...
        with patch("yousef_shtiwe_cli_core.models.fetch_api_models", return_value=["model-a"]) as mock_api, \
    ...
    def test_stale_cache_used_on_total_failure(self, tmp_path, monkeypatch):
        ...
        from yousef_shtiwe_cli_core.models import fetch_ollama_cloud_models, _save_ollama_cloud_cache
        ...
        with patch("yousef_shtiwe_cli_core.models.fetch_api_models", return_value=None), \
             patch("agent.models_dev.fetch_models_dev", return_value={}):
            result = fetch_ollama_cloud_models(force_refresh=True)
...
class TestOllamaCloudProvidersNew:
    ...
    def test_alias_resolves(self):
        from yousef_shtiwe_cli_core.providers import normalize_provider as np
        assert np("ollama") == "custom"  # bare "ollama" = local
        assert np("ollama-cloud") == "ollama-cloud"

    def test_label_override(self):
        from yousef_shtiwe_cli_core.providers import _LABEL_OVERRIDES
    ...
    def test_get_provider(self):
        from yousef_shtiwe_cli_core.providers import get_provider
