...
def hermes_home(tmp_path, monkeypatch):
    """Set up an isolated YOUSEF_HOME with minimal logs."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from yousef_shtiwe_cli_core.debug import _upload_paste_rs
        ...
        with patch("yousef_shtiwe_cli_core.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "yousef_shtiwe_cli_core.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from yousef_shtiwe_cli_core.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from yousef_shtiwe_cli_core.debug import upload_to_pastebin

        with patch("yousef_shtiwe_cli_core.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("yousef_shtiwe_cli_core.debug._upload_paste_rs",
             ...
             patch("yousef_shtiwe_cli_core.debug._upload_dpaste_com",
...
class TestReadFullLog:
    ...
    def test_reads_small_file(self, hermes_home):
        from yousef_shtiwe_cli_core.debug import _read_full_log
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, hermes_home):
        from yousef_shtiwe_cli_core.debug import collect_debug_report

        with patch("yousef_shtiwe_cli_core.dump.run_dump") as mock_dump:
    ...
    def test_report_includes_agent_log(self, hermes_home):
        ...
        with patch("yousef_shtiwe_cli_core.dump.run_dump"):
            report = collect_debug_report(log_lines=50)
...
class TestRunDebugShare:
    ...
    def test_share_uploads_three_pastes(self, hermes_home, capsys):
        ...
        with patch("yousef_shtiwe_cli_core.dump.run_dump") as mock_dump, \
             patch("yousef_shtiwe_cli_core.debug.upload_to_pastebin",
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from yousef_shtiwe_cli_core.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from yousef_shtiwe_cli_core.debug import delete_paste
        ...
        with patch("yousef_shtiwe_cli_core.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    def test_spawns_detached_process(self):
        from yousef_shtiwe_cli_core.debug import _schedule_auto_delete
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from yousef_shtiwe_cli_core.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("yousef_shtiwe_cli_core.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("yousef_shtiwe_cli_core.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, hermes_home, capsys):
        ...
        with patch("yousef_shtiwe_cli_core.dump.run_dump"), \
             ...
             patch("yousef_shtiwe_cli_core.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, hermes_home, capsys):
        ...
        with patch("yousef_shtiwe_cli_core.dump.run_dump"), \
             ...
             patch("yousef_shtiwe_cli_core.debug._schedule_auto_delete"):
            run_debug_share(args)
