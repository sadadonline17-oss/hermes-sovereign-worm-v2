...
import pytest
from yousef_shtiwe_cli_core.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from yousef_shtiwe_cli_core.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("yousef_shtiwe_cli_core.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `yousef setup` should reach setup.py, which prints headless guidance."""
        from yousef_shtiwe_cli_core.main import cmd_setup
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("yousef_shtiwe_cli_core.setup.ensure_hermes_home"),
            patch("yousef_shtiwe_cli_core.setup.load_config", return_value={}),
            patch("yousef_shtiwe_cli_core.setup.get_hermes_home", return_value="/tmp/.yousef"),
            patch("yousef_shtiwe_cli_core.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "yousef config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        ...
        from yousef_shtiwe_cli_core.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("yousef_shtiwe_cli_core.main._has_any_provider_configured", return_value=False),
            patch("yousef_shtiwe_cli_core.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
    ...
    def test_returning_user_terminal_menu_choice_dispatches_terminal_section(self, tmp_path):
        ...
        with (
            ...
            patch("yousef_shtiwe_cli_core.auth.get_active_provider", return_value=None),
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`yousef setup tts` should parse and dispatch like other setup sections."""
        from yousef_shtiwe_cli_core import main as main_mod
        ...
        monkeypatch.setattr("sys.argv", ["yousef", "setup", "tts"])
