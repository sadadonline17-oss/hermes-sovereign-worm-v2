...
``yousef_shtiwe_cli_core.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``yousef_shtiwe_cli_core.model_switch.switch_model`` was missing the
...
def _run_opencode_switch(
    ...
    with (
        patch("yousef_shtiwe_cli_core.model_switch.resolve_alias", return_value=None),
        patch("yousef_shtiwe_cli_core.model_switch.list_provider_models", return_value=[]),
        patch(
        ...
        patch(
            "yousef_shtiwe_cli_core.models.validate_requested_model",
            return_value=_MOCK_VALIDATION,
        ),
        patch("yousef_shtiwe_cli_core.model_switch.get_model_info", return_value=None),
        patch("yousef_shtiwe_cli_core.model_switch.get_model_capabilities", return_value=None),
        patch("yousef_shtiwe_cli_core.models.detect_provider_for_model", return_value=None),
    ):
        return switch_model(
            raw_input=raw_input,
