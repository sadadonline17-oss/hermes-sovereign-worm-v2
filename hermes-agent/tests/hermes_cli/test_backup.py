...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.yousef directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "yousef_state.db").write_bytes(b"fake-state")
    ...
    # yousef-sovereign-core repo (should be EXCLUDED)
    (root / "yousef-sovereign-core").mkdir(exist_ok=True)
    (root / "yousef-sovereign-core" / "run_agent.py").write_text("# big file\n")
    (root / "yousef-sovereign-core" / ".git").mkdir()
    (root / "yousef-sovereign-core" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_yousef_core(self):
        from yousef_shtiwe_cli_core.backup import _should_exclude
        assert _should_exclude(Path("yousef-sovereign-core/run_agent.py"))
        assert _should_exclude(Path("yousef-sovereign-core/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        hermes_home = tmp_path / ".yousef"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)

        monkeypatch.setenv("YOUSEF_HOME", str(hermes_home))
    ...
    def test_excludes_yousef_core(self, tmp_path, monkeypatch):
        """Backup does NOT include yousef-sovereign-core/ directory."""
        hermes_home = tmp_path / ".yousef"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "yousef-sovereign-core" in n]
            assert agent_files == [], f"yousef-sovereign-core files leaked into backup: {agent_files}"
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from yousef_shtiwe_cli_core.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only yousef_state.db (old wrong name) is rejected."""
        from yousef_shtiwe_cli_core.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["yousef_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_strips_hermes_prefix(self, tmp_path, monkeypatch):
        """Import strips .yousef/ prefix if all entries share it."""
        hermes_home = tmp_path / ".yousef"
        hermes_home.mkdir()
        monkeypatch.setenv("YOUSEF_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".yousef/config.yaml": "model: test\n",
            ".yousef/skills/a/SKILL.md": "# A\n",
        })
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".yousef"
        src_home.mkdir(parents=True)
        _make_hermes_tree(src_home)

        monkeypatch.setenv("YOUSEF_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from yousef_shtiwe_cli_core.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".yousef"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("YOUSEF_HOME", str(dst_home))
        ...
        # yousef-sovereign-core should NOT be present
        assert not (dst_home / "yousef-sovereign-core").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from yousef_shtiwe_cli_core.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_hermes(self):
        """Detects .yousef/ prefix wrapping all entries."""
        import io
        from yousef_shtiwe_cli_core.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".yousef/config.yaml", "test")
            zf.writestr(".yousef/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".yousef/"
...
class TestBackupEdgeCases:
    def test_nonexistent_hermes_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "nonexistent" / ".yousef"
        monkeypatch.setenv("YOUSEF_HOME", str(fake_home))
...
class TestProfileRestoration:
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import yousef_shtiwe_cli_core.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "yousef_shtiwe_cli_core.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from yousef_shtiwe_cli_core.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def hermes_home(self, tmp_path):
        """Create a fake YOUSEF_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, hermes_home):
        from yousef_shtiwe_cli_core.backup import create_quick_snapshot
    ...
    def test_list_snapshots(self, hermes_home):
        from yousef_shtiwe_cli_core.backup import create_quick_snapshot, list_quick_snapshots
    ...
    def test_restore_config(self, hermes_home):
        from yousef_shtiwe_cli_core.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(hermes_home=hermes_home)
    ...
    def test_restore_nonexistent(self, hermes_home):
        from yousef_shtiwe_cli_core.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", hermes_home=hermes_home) is False

    def test_auto_prune(self, hermes_home):
        from yousef_shtiwe_cli_core.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
    ...
    def test_manual_prune(self, hermes_home):
        from yousef_shtiwe_cli_core.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", hermes_home=hermes_home)
