...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "yousef"
        assert config.workspace_id == "yousef"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.yousef/honcho.json
        monkeypatch.setenv("YOUSEF_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"yousef": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"yousef": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"yousef": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="yousef-sovereign-core"
        ):
            result = config.resolve_session_name("/home/user/yousef-sovereign-core/subdir")
        assert result == "yousef-sovereign-core"
...
class TestResolveConfigPath:
    def test_prefers_hermes_home_when_exists(self, tmp_path):
        ...
        with patch.dict(os.environ, {"YOUSEF_HOME": str(hermes_home)}):
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        hermes_home = tmp_path / "yousef"
        hermes_home.mkdir()
        # No honcho.json in YOUSEF_HOME — also isolate ~/.yousef so
    ...
    def test_falls_back_to_global_without_hermes_home_env(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("YOUSEF_HOME", None)
            result = resolve_config_path()
        assert result == GLOBAL_CONFIG_PATH
...
class TestResolveActiveHost:
    def test_default_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=True):
            ...
            os.environ.pop("YOUSEF_HOME", None)
            assert resolve_active_host() == "yousef"
    ...
    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("yousef_shtiwe_cli_core.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "hermes.coder"

    def test_default_profile_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("yousef_shtiwe_cli_core.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "yousef"

    def test_custom_profile_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("yousef_shtiwe_cli_core.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "yousef"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            # Temporarily remove yousef_shtiwe_cli_core.profiles to simulate import failure
            saved = sys.modules.get("yousef_shtiwe_cli_core.profiles")
            sys.modules["yousef_shtiwe_cli_core.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "yousef"
            finally:
                if saved is not None:
                    sys.modules["yousef_shtiwe_cli_core.profiles"] = saved
                else:
                    sys.modules.pop("yousef_shtiwe_cli_core.profiles", None)
...
class TestProfileScopedConfig:
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "yousef": {"aiPeer": "yousef", "peerName": "alice"},
                "hermes.coder": {
                    "aiPeer": "hermes.coder",
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"yousef": {"enabled": True, "aiPeer": "yousef"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"yousef": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_hermes_config_timeout_override_used_when_config_timeout_missing(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("yousef_shtiwe_cli_core.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_hermes_request_timeout_alias_used(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("yousef_shtiwe_cli_core.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
