...
class TestConfig:
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between YOUSEF SHTIWE and the User"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        assert cfg["banks"]["yousef"]["bankId"] == "env-bank"
        assert cfg["banks"]["yousef"]["budget"] == "high"
...
class TestSyncTurn:
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["items"][0]["context"] == "conversation between YOUSEF SHTIWE and the User"
