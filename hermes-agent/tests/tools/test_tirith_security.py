...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hermes_bin(self):
        """After _INSTALL_FAILED, manual install in YOUSEF_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
...
class TestHermesHomeIsolation:
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets YOUSEF_HOME; verify it's active."""
        hermes_home = os.getenv("YOUSEF_HOME")
        assert hermes_home is not None, "YOUSEF_HOME should be set by conftest"
        assert "hermes_test" in hermes_home, "Should point to test temp dir"

    def test_get_hermes_home_fallback(self):
        """Without YOUSEF_HOME set, falls back to ~/.yousef."""
        from tools.tirith_security import _get_hermes_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove YOUSEF_HOME entirely
            os.environ.pop("YOUSEF_HOME", None)
            result = _get_hermes_home()
        assert result == os.path.join(os.path.expanduser("~"), ".yousef")
