...
def _restore_tool_modules():
    original_hermes_home = os.environ.get("YOUSEF_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "yousef_shtiwe_cli_core"
        or name.startswith("yousef_shtiwe_cli_core.")
        or name == "modal"
    ...
    try:
        ...
        if original_hermes_home is None:
            ...
            os.environ["YOUSEF_HOME"] = original_hermes_home
        _reset_modules(("tools", "yousef_shtiwe_cli_core", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    yousef_shtiwe_cli_core = types.ModuleType("yousef_shtiwe_cli_core")
    yousef_shtiwe_cli_core.__path__ = []  # type: ignore[attr-defined]
    sys.modules["yousef_shtiwe_cli_core"] = yousef_shtiwe_cli_core
    hermes_home = tmp_path / "hermes-home"
    os.environ["YOUSEF_HOME"] = str(hermes_home)
    sys.modules["yousef_shtiwe_cli_core.config"] = types.SimpleNamespace(
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="yousef-sovereign-core", create_if_missing=create_if_missing)
