...
from yousef_constants import get_default_hermes_root, is_container
...
class TestGetDefaultHermesRoot:
    ...
    def test_no_hermes_home_returns_native(self, tmp_path, monkeypatch):
        """When YOUSEF_HOME is not set, returns ~/.yousef."""
        monkeypatch.delenv("YOUSEF_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        assert get_default_hermes_root() == tmp_path / ".yousef"

    def test_hermes_home_is_native(self, tmp_path, monkeypatch):
        """When YOUSEF_HOME = ~/.yousef, returns ~/.yousef."""
        native = tmp_path / ".yousef"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("YOUSEF_HOME", str(native))
        assert get_default_hermes_root() == native

    def test_hermes_home_is_profile(self, tmp_path, monkeypatch):
        """When YOUSEF_HOME is a profile under ~/.yousef, returns ~/.yousef."""
        native = tmp_path / ".yousef"
    ...
    def test_hermes_home_is_docker(self, tmp_path, monkeypatch):
        """When YOUSEF_HOME points outside ~/.yousef (Docker), returns YOUSEF_HOME."""
    ...
    def test_hermes_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any YOUSEF_HOME outside ~/.yousef is treated as the root."""
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (YOUSEF_HOME=<root>/profiles/<name>),
