...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``yousef_shtiwe_cli_core.config._sanitize_env_lines`` which
    ...
    if not path.exists():
        return
    try:
        from yousef_shtiwe_cli_core.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    Behavior:
    - `~/.yousef/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("YOUSEF_HOME", Path.home() / ".yousef"))
