"""
YOUSEF SHTIWE — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m yousef_shtiwe_cli_core.main web          # Start on http://127.0.0.1:9119
    python -m yousef_shtiwe_cli_core.main web --port 8080
"""
...
try:
    ...
    raise SystemExit(
        ...
        "Run 'hermes web' to auto-install, or: pip install yousef-sovereign-core[web]"
    )

WEB_DIST = Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="YOUSEF SHTIWE", version=__version__)
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.yousef/.anthropic_oauth.json`` — Hermes-managed PKCE flow
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from yousef_shtiwe_cli_core import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in ("anthropic", "claude-code"):
        ...
        try:
            from yousef_shtiwe_cli_core.auth import clear_provider_auth
...
#          → persists to ~/.yousef/.anthropic_oauth.json AND credential pool
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    from yousef_shtiwe_cli_core import auth as hauth
    if provider_id == "nous":
        from yousef_shtiwe_cli_core.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    from yousef_shtiwe_cli_core.auth import _poll_for_token, refresh_nous_oauth_from_state
...
async def get_logs(
    ...
    from yousef_shtiwe_cli_core.logs import _read_tail, LOG_FILES
    ...
    try:
        from yousef_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from yousef_shtiwe_cli_core.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from yousef_shtiwe_cli_core.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "Shadow Red",  "description": "Hacker Noir - Blood Red — the canonical Hermes look"},
...
def _discover_user_themes() -> list:
    """Scan ~/.yousef/dashboard-themes/*.yaml for user-created themes."""
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as yousef_shtiwe_cli_core.plugins):
    1. User plugins:    ~/.yousef/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.yousef/plugins/  (only if HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    if os.environ.get("HERMES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".yousef" / "plugins", "project"))
