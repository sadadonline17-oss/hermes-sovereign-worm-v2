"""``yousef shtiwe plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.yousef/plugins/``.
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from yousef_shtiwe_cli_core.config import get_env_value, save_env_value  # noqa: F811
...
def cmd_install(identifier: str, force: bool = False) -> None:
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]yousef shtiwe plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid Sovereign plugin."
        )
    ...
    console.print("[dim]  yousef gateway restart[/dim]")
    console.print()
...
def cmd_list() -> None:
    ...
    if not dirs:
        ...
        console.print("[dim]Install with:[/dim] yousef shtiwe plugins install owner/repo")
        return
    ...
    for d in dirs:
        ...
        # Check if it's a git repo (installed via yousef shtiwe plugins install)
        if (d / ".git").exists():
            source = "git"
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] yousef shtiwe plugins")
    console.print("[dim]Enable/disable:[/dim] yousef shtiwe plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch yousef shtiwe plugins subcommands."""
