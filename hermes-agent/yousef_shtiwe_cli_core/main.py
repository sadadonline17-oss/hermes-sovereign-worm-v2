...
Usage:
    ...
    yousef chat                # Interactive chat
    yousef gateway             # Run gateway in foreground
    yousef gateway start       # Start gateway as service
    yousef gateway stop        # Stop gateway service
    yousef gateway status      # Show gateway status
    yousef gateway install     # Install gateway service
    yousef gateway uninstall   # Uninstall gateway service
    yousef setup               # Interactive setup wizard
    hermes logout              # Clear stored authentication
    hermes status              # Show status of all components
    hermes cron                # Manage cron jobs
    hermes cron list           # List cron jobs
    hermes cron status         # Check if cron scheduler is running
    yousef doctor              # Check configuration and dependencies
    ...
    hermes uninstall           Uninstall YOUSEF SHTIWE
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (hermes tools, yousef setup, yousef model) use
...
#
# Many modules cache YOUSEF_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("YOUSEF_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.yousef/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set YOUSEF_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    if profile_name is None:
        try:
            from yousef_constants import get_default_hermes_root
    ...
    # 3. If we found a profile, resolve and set YOUSEF_HOME
    if profile_name is not None:
        try:
            from yousef_shtiwe_cli_core.profiles import resolve_profile_env
            ...
            return
        os.environ["YOUSEF_HOME"] = hermes_home
...
# Load .env from ~/.yousef/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from yousef_shtiwe_cli_core.config import get_hermes_home
from yousef_shtiwe_cli_core.env_loader import load_hermes_dotenv
load_hermes_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `hermes` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from yousef_logging import setup_logging as _setup_logging
...
try:
    from yousef_shtiwe_cli_core.config import load_config as _load_config_early
    from yousef_constants import apply_ipv4_preference as _apply_ipv4
...
from yousef_shtiwe_cli_core import __version__, __release_date__
from yousef_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from yousef_shtiwe_cli_core.config import get_env_path, get_hermes_home, load_config
    from yousef_shtiwe_cli_core.auth import get_auth_status

    # Determine whether Hermes itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from yousef_shtiwe_cli_core.config import DEFAULT_CONFIG
    ...
    from yousef_shtiwe_cli_core.auth import PROVIDER_REGISTRY
...
def _resolve_last_cli_session() -> Optional[str]:
    ...
    try:
        from yousef_state import SessionDB
        db = SessionDB()
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'yousef')
    """
    import shutil
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        ...
        print()
        print("  Run:  yousef setup")
        print()

        from yousef_shtiwe_cli_core.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'yousef setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from yousef_shtiwe_cli_core.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["YOUSEF_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["YOUSEF_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from yousef_shtiwe_cli_core.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: yousef gateway")
            return
...
def cmd_setup(args):
    ...
    from yousef_shtiwe_cli_core.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``yousef model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from yousef_shtiwe_cli_core.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from yousef_shtiwe_cli_core.config import get_compatible_custom_providers, load_config, get_env_value
    ...
    from yousef_shtiwe_cli_core.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.yousef/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.yousef/.env if the active provider is not 'custom'.
    ...
    """
    from yousef_shtiwe_cli_core.config import get_env_value, save_env_value, load_config
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from yousef_shtiwe_cli_core.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from yousef_shtiwe_cli_core.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from yousef_shtiwe_cli_core.config import get_env_value, save_env_value
    ...
    from yousef_shtiwe_cli_core.models import model_ids, get_pricing_for_provider
    ...
    if selected:
        ...
        from yousef_shtiwe_cli_core.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from yousef_shtiwe_cli_core.config import get_env_value, load_config, save_config, save_env_value
    from yousef_shtiwe_cli_core.nous_subscription import prompt_enable_tool_gateway
    import argparse
    ...
    from yousef_shtiwe_cli_core.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from yousef_shtiwe_cli_core.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from yousef_shtiwe_cli_core.codex_models import get_codex_model_ids
    import argparse
    ...
    if not _codex_token:
        try:
            from yousef_shtiwe_cli_core.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from yousef_shtiwe_cli_core.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    from yousef_shtiwe_cli_core.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from yousef_shtiwe_cli_core.auth import _save_model_choice, deactivate_provider
    from yousef_shtiwe_cli_core.config import get_env_value, load_config, save_config
    ...
    from yousef_shtiwe_cli_core.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `yousef model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from yousef_shtiwe_cli_core.curses_ui import flush_stdin
        flush_stdin()
        print()
...
def _model_flow_copilot(config, current_model=""):
    ...
    from yousef_shtiwe_cli_core.config import save_env_value, load_config, save_config
    from yousef_shtiwe_cli_core.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from yousef_shtiwe_cli_core.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from yousef_shtiwe_cli_core.copilot_auth import validate_copilot_token
...
def _model_flow_kimi(config, current_model=""):
    ...
    from yousef_shtiwe_cli_core.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    """
    from yousef_shtiwe_cli_core.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from yousef_shtiwe_cli_core.config import load_config, save_config, get_env_value, save_env_value
    from yousef_shtiwe_cli_core.models import _PROVIDER_MODELS
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from yousef_shtiwe_cli_core.config import get_env_value, save_env_value, load_config, save_config
    from yousef_shtiwe_cli_core.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
    ...
    if provider_id == "ollama-cloud":
        from yousef_shtiwe_cli_core.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from yousef_shtiwe_cli_core.config import (
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from yousef_constants import display_hermes_home as _dhh_fn
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from yousef_shtiwe_cli_core.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'yousef model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    ...
    from yousef_shtiwe_cli_core.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from yousef_shtiwe_cli_core.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from yousef_shtiwe_cli_core.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from yousef_shtiwe_cli_core.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from yousef_shtiwe_cli_core.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from yousef_shtiwe_cli_core.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from yousef_shtiwe_cli_core.doctor import run_doctor
    run_doctor(args)


def cmd_dump(args):
    """Dump setup summary for support/debugging."""
    from yousef_shtiwe_cli_core.dump import run_dump
    run_dump(args)


def cmd_debug(args):
    """Debug tools (share report, etc.)."""
    from yousef_shtiwe_cli_core.debug import run_debug
    run_debug(args)


def cmd_config(args):
    """Configuration management."""
    from yousef_shtiwe_cli_core.config import config_command
    config_command(args)


def cmd_backup(args):
    """Back up Sovereign home directory to a zip file."""
    if getattr(args, "quick", False):
        from yousef_shtiwe_cli_core.backup import run_quick_backup
        run_quick_backup(args)
    else:
        from yousef_shtiwe_cli_core.backup import run_backup
        run_backup(args)


def cmd_import(args):
    """Restore a Hermes backup from a zip file."""
    from yousef_shtiwe_cli_core.backup import run_import
    run_import(args)


def cmd_version(args):
    """Show version."""
    print(f"YOUSEF SHTIWE v{__version__} ({__release_date__})")
    ...
    try:
        from yousef_shtiwe_cli_core.banner import check_for_updates
        from yousef_shtiwe_cli_core.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall YOUSEF SHTIWE."""
    _require_tty("uninstall")
    from yousef_shtiwe_cli_core.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from yousef_constants import get_hermes_home
...
def _update_via_zip(args):
    """Update YOUSEF SHTIWE by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/SovereignCore/yousef-sovereign-core/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="hermes-update-")
        zip_path = os.path.join(tmp_dir, f"yousef-sovereign-core-{branch}.zip")
        ...
        # GitHub ZIPs extract to yousef-sovereign-core-<branch>/
        extracted = os.path.join(tmp_dir, f"yousef-sovereign-core-{branch}")
...
OFFICIAL_REPO_URLS = {
    "https://github.com/SovereignCore/yousef-sovereign-core.git",
    "git@github.com:SovereignCore/yousef-sovereign-core.git",
    "https://github.com/SovereignCore/yousef-sovereign-core",
    "git@github.com:SovereignCore/yousef-sovereign-core",
}
OFFICIAL_REPO_URL = "https://github.com/SovereignCore/yousef-sovereign-core.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from SovereignCore/yousef-sovereign-core.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/SovereignCore/yousef-sovereign-core.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/SovereignCore/yousef-sovereign-core.git' to add later.")
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "yousef-sovereign-core[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update YOUSEF SHTIWE to the latest version."""
    import shutil
    from yousef_shtiwe_cli_core.config import is_managed, managed_error

    if is_managed():
        managed_error("update YOUSEF SHTIWE")
        return
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/SovereignCore/yousef-sovereign-core/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_hermes_home added to yousef_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload yousef_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_hermes_home() added since the last release.
        try:
            import importlib
            import yousef_constants as _hc
        ...
        try:
            from yousef_shtiwe_cli_core.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'yousef config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: yousef config migrate")
            else:
                print()
                print("Skipped. Run 'yousef config migrate' later to configure.")
        else:
        ...
        try:
            from yousef_shtiwe_cli_core.gateway import (
            ...
            if is_macos():
                try:
                    from yousef_shtiwe_cli_core.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: yousef gateway run")
        ...
        print("  yousef model              # Select provider and model")
...
def cmd_profile(args):
    ...
    from yousef_shtiwe_cli_core.profiles import (
    ...
    from yousef_constants import display_hermes_home
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.yousef)")
            else:
        ...
        name = args.profile_name
        from yousef_shtiwe_cli_core.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from yousef_shtiwe_cli_core.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from yousef_shtiwe_cli_core.profiles import rename_profile
        try:
        ...
        from yousef_shtiwe_cli_core.profiles import export_profile
        name = args.profile_name
        ...
        from yousef_shtiwe_cli_core.profiles import import_profile
        try:
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install yousef-sovereign-core[web]")
        sys.exit(1)

    if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
        sys.exit(1)

    from yousef_shtiwe_cli_core.web_server import start_server
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from yousef_shtiwe_cli_core.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from yousef_shtiwe_cli_core.logs import tail_log, list_logs
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="yousef",
        description="YOUSEF SHTIWE - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    hermes                        Start interactive chat
    yousef chat -q "Hello"        Single query mode
    hermes -c                     Resume the most recent session
    hermes -c "my project"        Resume a session by name (latest in lineage)
    hermes --resume <session_id>  Resume a specific session by ID
    yousef setup                  Run setup wizard
    hermes logout                 Clear stored authentication
    hermes auth add <provider>    Add a pooled credential
    hermes auth list              List pooled credentials
    hermes auth remove <p> <t>    Remove pooled credential by index, id, or label
    hermes auth reset <provider>  Clear exhaustion status for a provider
    yousef model                  Select default model
    yousef config                 View configuration
    yousef config edit            Edit config in $EDITOR
    yousef config set model gpt-4 Set a config value
    yousef gateway                Run messaging gateway
    hermes -s yousef-sovereign-core-dev,github-auth
    hermes -w                     Start in isolated git worktree
    yousef gateway install        Install gateway background service
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with YOUSEF SHTIWE"
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure YOUSEF SHTIWE with an interactive wizard. "
                    "Run a specific section: yousef setup model|tts|terminal|gateway|tools|agent"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of YOUSEF SHTIWE components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with YOUSEF SHTIWE setup"
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for YOUSEF SHTIWE. Use 'hermes debug share' to "
...
""",
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Sovereign home directory to a zip file",
        description="Create a zip archive of your entire Hermes configuration, "
                    "skills, sessions, and data (excludes the yousef-sovereign-core codebase). "
    ...
    import_parser = subparsers.add_parser(
        ...
        description="Extract a previously created Hermes backup into your "
                    "Sovereign home directory, restoring configuration, skills, "
                    "sessions, and data"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage YOUSEF SHTIWE configuration"
    )
    ...
    def cmd_pairing(args):
        from yousef_shtiwe_cli_core.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.yousef/skills/.bundled_manifest) "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from yousef_shtiwe_cli_core.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from yousef_shtiwe_cli_core.skills_hub import skills_command
            skills_command(args)
    ...
    def cmd_plugins(args):
        from yousef_shtiwe_cli_core.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from yousef_constants import get_hermes_home, display_hermes_home
            ...
            from yousef_shtiwe_cli_core.memory_setup import memory_command
            memory_command(args)
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from yousef_shtiwe_cli_core.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from yousef_shtiwe_cli_core.tools_config import tools_command
            tools_command(args)
    ...
    def cmd_mcp(args):
        from yousef_shtiwe_cli_core.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            import shutil
            hermes_bin = shutil.which("yousef")
            if hermes_bin:
                os.execvp(hermes_bin, ["yousef", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "yousef_shtiwe_cli_core.main", "--resume", selected_id],
                )
    ...
    def cmd_claw(args):
        from yousef_shtiwe_cli_core.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update YOUSEF SHTIWE to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall YOUSEF SHTIWE",
        description="Remove YOUSEF SHTIWE from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run YOUSEF SHTIWE as an ACP (Agent Client Protocol) server",
        description="Start YOUSEF SHTIWE in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch YOUSEF SHTIWE as an ACP server."""
        try:
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the YOUSEF SHTIWE web dashboard for managing config, API keys, and sessions",
    )
...
""",
    ...
    from yousef_shtiwe_cli_core.config import get_container_exec_info
