...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional skill(s) from Sovereign Core Injection[/]")
    c.print()
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This is an official optional skill maintained by Sovereign Core Injection.[/]\n\n"
                "It ships with yousef-sovereign-core but is not activated by default.\n"
...
def skills_command(args) -> None:
    """Router for `hermes skills <subcommand>` — called from yousef_shtiwe_cli_core/main.py."""
