...
Shows the status of all YOUSEF SHTIWE components.
"""
...
from yousef_shtiwe_cli_core.auth import AuthError, resolve_provider
from yousef_shtiwe_cli_core.colors import Colors, color
from yousef_shtiwe_cli_core.config import get_env_path, get_env_value, get_hermes_home, load_config
from yousef_shtiwe_cli_core.models import provider_label
from yousef_shtiwe_cli_core.nous_subscription import get_nous_subscription_features
from yousef_shtiwe_cli_core.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all YOUSEF SHTIWE components."""
    ...
    try:
        from yousef_shtiwe_cli_core.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: yousef model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: yousef model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: yousef model)"
    ...
    if _is_termux():
        try:
            from yousef_shtiwe_cli_core.gateway import find_gateway_pids
        ...
        if gateway_pids:
            ...
            print("  Start with:   yousef gateway")
        ...
        if is_container():
            ...
            try:
                from yousef_shtiwe_cli_core.gateway import get_service_name
        ...
        from yousef_shtiwe_cli_core.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    print(color("  Run 'yousef doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'yousef setup' to configure", Colors.DIM))
    print()
