"""
Interactive setup wizard for YOUSEF SHTIWE.
...
Config files are stored in ~/.yousef/ for easy access.
"""
...
from yousef_shtiwe_cli_core.nous_subscription import get_nous_subscription_features
from tools.tool_backend_helpers import managed_nous_tools_enabled
from yousef_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://yousef-sovereign-core.nousresearch.com/docs"
...
from yousef_shtiwe_cli_core.colors import Colors, color
...
from yousef_shtiwe_cli_core.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("  yousef config set model.provider custom")
    print_info("  yousef config set model.base_url http://localhost:8080/v1")
    print_info("  yousef config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'yousef setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from yousef_shtiwe_cli_core.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from yousef_shtiwe_cli_core.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'yousef setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'yousef setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'yousef setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'yousef setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'yousef setup tools' to configure them,"
        )
        from yousef_constants import display_hermes_home as _dhh
    ...
    print()
    print(f"   {color('yousef setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('yousef setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('yousef setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('yousef setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('yousef setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('yousef config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('yousef config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('yousef config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('yousef', Colors.GREEN)}              Start chatting")
    print(f"   {color('yousef gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('yousef doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `hermes tools` and `yousef setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``yousef model``)
    ...
    provider added to ``yousef model`` is automatically available here.
    ...
    # Delegate to the shared yousef model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from yousef_shtiwe_cli_core.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: yousef model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from yousef_shtiwe_cli_core.auth_commands import auth_add_command
    ...
    if _vision_needs_setup:
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'yousef setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.yousef/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"yousef setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'yousef setup tts')."""
    _setup_tts_provider(config)
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `yousef setup agent` later to customize.")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://yousef-sovereign-core.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_weixin():
    ...
    from yousef_shtiwe_cli_core.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from yousef_shtiwe_cli_core.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from yousef_shtiwe_cli_core.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from yousef_shtiwe_cli_core.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from yousef_shtiwe_cli_core.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from yousef_shtiwe_cli_core.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from yousef_shtiwe_cli_core.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from yousef_shtiwe_cli_core.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    from yousef_shtiwe_cli_core.gateway import _PLATFORMS
    qq_platform = next((p for p in _PLATFORMS if p["key"] == "qqbot"), None)
    if qq_platform:
        from yousef_shtiwe_cli_core.gateway import _setup_standard_platform
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://yousef-sovereign-core.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://yousef-sovereign-core.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  yousef config edit")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'yousef setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     yousef config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: yousef gateway install")
            else:
                print_info("  You can install later: yousef gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo yousef gateway install --system")
                print_info("  Or run in foreground:  yousef gateway")
        else:
            from yousef_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   yousef gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   yousef gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `yousef setup tools` and `hermes tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from yousef_shtiwe_cli_core.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from yousef_shtiwe_cli_core.auth import get_active_provider
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      yousef setup           — full or quick (auto-detected)
      yousef setup model     — just model/provider
      yousef setup tts       — just text-to-speech
      yousef setup terminal  — just terminal backend
      yousef setup gateway   — just messaging platforms
      yousef setup tools     — just tool configuration
      yousef setup agent     — just agent settings
    """
    ...
    print(
        color(
            "│  Let's configure your YOUSEF SHTIWE installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if choice == 0:
            ...
            print_info("Exiting. Run 'yousef setup' again when ready.")
            return
        elif 2 <= choice <= 6:
    ...
    print()
    print_info("You can edit these files directly or use 'yousef config edit'")
...
def _resolve_hermes_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``yousef chat`` in a fresh process."""
    hermes_bin = shutil.which("yousef")
    if hermes_bin:
        return [hermes_bin, "chat"]

    try:
        if importlib.util.find_spec("yousef_shtiwe_cli_core") is not None:
            return [sys.executable, "-m", "yousef_shtiwe_cli_core.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch yousef chat now?", True):
        return

    chat_argv = _resolve_hermes_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Hermes automatically. Run 'yousef chat' manually.")
        return
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'yousef setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    yousef setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  yousef setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'yousef setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Link YOUSEF SHTIWE to Infiltration Gateways to chat from anywhere.")
        print_info("You can configure these later with 'yousef setup gateway'.")
