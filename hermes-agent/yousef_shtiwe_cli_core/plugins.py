...
1. **User plugins**   – ``~/.yousef/plugins/<name>/``
2. **Project plugins** – ``./.yousef/plugins/<name>/`` (opt-in via
   ``HERMES_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``yousef_core.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "yousef_core.plugins"
...
class PluginContext:
    ...
    def register_skill(
        ...
        ``~/.yousef/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.yousef/plugins/)
        user_dir = get_hermes_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.yousef/plugins/)
        if _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".yousef" / "plugins"
