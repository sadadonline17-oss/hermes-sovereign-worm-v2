...
Skins are defined as YAML files in ~/.yousef/skins/ or as built-in presets.
...
.. code-block:: yaml
    ...
    colors:
      banner_border: "#8B0000"            # Panel border color
      banner_title: "#FF0000"             # Panel title text color
      banner_accent: "#FF4500"            # Section headers (Available Tools, etc.)
      banner_dim: "#440000"               # Dim/muted text (separators, labels)
      banner_text: "#E0E0E0"              # Body text (tool names, skill names)
      ui_accent: "#FF4500"               # General UI accent
      ui_label: "#FF0000"                # UI labels
      ui_ok: "#00FF41"                   # Success indicators
      ui_error: "#FF0000"                # Error indicators
      ui_warn: "#FF0000"                 # Warning indicators
      prompt: "#E0E0E0"                  # Prompt text color
      input_rule: "#8B0000"              # Input area horizontal rule
      response_border: "#FF0000"         # Response box border (ANSI)
      session_label: "#8B0000"           # Session label color
      session_border: "#222222"          # Session ID dim color
      status_bar_bg: "#110000"          # TUI status/usage bar background
      voice_status_bg: "#110000"        # TUI voice status background
      completion_menu_bg: "#110000"      # Completion menu background
      completion_menu_current_bg: "#440000"  # Active completion row background
      completion_menu_meta_bg: "#110000"     # Completion meta column background
      completion_menu_meta_current_bg: "#440000"  # Active completion meta background
...
.. code-block:: python
    ...
    from yousef_shtiwe_cli_core.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FF0000"
    print(skin.get_branding("agent_name"))  # "YOUSEF SHTIWE"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.yousef/skins/
...
Drop a YAML file in ``~/.yousef/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces yousef_core_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces YOUSEF_PREDATOR_SYMBOL)
...
_active_skin_name: str = "shadow"


def _skins_dir() -> Path:
    """User skins directory."""
    # Add project-local skins directory to the search path
    local_skins = Path(__file__).parent / "skins"
    return local_skins if local_skins.exists() else get_hermes_home() / "skins"
...
def get_prompt_toolkit_style_overrides() -> Dict[str, str]:
    ...
    prompt = skin.get_color("prompt", "#E0E0E0")
    input_rule = skin.get_color("input_rule", "#8B0000")
    title = skin.get_color("banner_title", "#FF0000")
    text = skin.get_color("banner_text", prompt)
    dim = skin.get_color("banner_dim", "#555555")
    label = skin.get_color("ui_label", title)
    warn = skin.get_color("ui_warn", "#FF8C00")
    error = skin.get_color("ui_error", "#FF6B6B")
    status_bg = skin.get_color("status_bar_bg", "#110000")
    voice_bg = skin.get_color("voice_status_bg", status_bg)
    menu_bg = skin.get_color("completion_menu_bg", "#110000")
    menu_current_bg = skin.get_color("completion_menu_current_bg", "#440000")
