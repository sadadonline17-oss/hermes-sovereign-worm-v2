"""
Unified tool configuration for YOUSEF SHTIWE.

`hermes tools` and `yousef setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.yousef/config.yaml under
...
from yousef_shtiwe_cli_core.nous_subscription import (
...
from yousef_shtiwe_cli_core.cli_output import (  # noqa: E402 — late import block
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from yousef_shtiwe_cli_core.plugins import discover_plugins, get_plugin_toolsets
...
from yousef_shtiwe_cli_core.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hermes_home()}/yousef-sovereign-core && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in yousef-sovereign-core directory)")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `hermes tools` and `yousef setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("☠ WORM V2 Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://yousef-sovereign-core.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color("  Changes take effect on next 'yousef' or gateway restart.", Colors.DIM))
    print()
