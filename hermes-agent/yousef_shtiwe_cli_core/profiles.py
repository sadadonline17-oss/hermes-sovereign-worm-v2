...
Each profile is a fully independent YOUSEF_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.yousef/profiles/<name>/`` by default.

The "default" profile is ``~/.yousef`` itself — backward compatible,
...
_PROFILE_DIRS = [
    ...
    # See yousef_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.yousef) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "yousef-sovereign-core",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "yousef", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current YOUSEF_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where YOUSEF_HOME points outside
    ``~/.yousef``, profiles live under ``YOUSEF_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) YOUSEF_HOME path.

    In standard deployments this is ``~/.yousef``.
    In Docker/custom deployments where YOUSEF_HOME is outside ``~/.yousef``
    (e.g. ``/opt/data``), returns YOUSEF_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.yousef
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its YOUSEF_HOME directory."""
    if name == "default":
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.yousef)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches YOUSEF_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "YOUSEF_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.yousef).\n"
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set YOUSEF_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("YOUSEF_HOME")
    try:
        os.environ["YOUSEF_HOME"] = str(profile_dir)
        from yousef_shtiwe_cli_core.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["YOUSEF_HOME"] = old_home
        elif "YOUSEF_HOME" in os.environ:
            del os.environ["YOUSEF_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.yousef/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from YOUSEF_HOME.

    Returns ``"default"`` if YOUSEF_HOME is not set or points to ``~/.yousef``.
    Returns the profile name if YOUSEF_HOME points into ``~/.yousef/profiles/<name>``.
    Returns ``"custom"`` if YOUSEF_HOME is set to an unrecognized path.
    """
    from yousef_constants import get_hermes_home
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.yousef itself — its parent is ~/ and its
        # directory name is ".yousef", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.yousef itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.yousef). "
...
def generate_bash_completion() -> str:
    ...
    return '''# YOUSEF SHTIWE profile completion
# Add to ~/.bashrc: eval "$(hermes completion bash)"

_hermes_profiles() {
    local profiles_dir="$HOME/.yousef/profiles"
...
# YOUSEF SHTIWE profile completion
# Add to ~/.zshrc: eval "$(hermes completion zsh)"

_hermes() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.yousef/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.yousef/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a YOUSEF_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the YOUSEF_HOME environment variable.
    """
