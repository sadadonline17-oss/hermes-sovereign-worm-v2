...
Handles: yousef gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when yousef_constants is cached from a pre-update version during `hermes update`.
from yousef_shtiwe_cli_core.setup import (
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    patterns = [
        "yousef_shtiwe_cli_core.main gateway",
        "yousef_shtiwe_cli_core.main --profile",
        "yousef_shtiwe_cli_core.main -p",
        "yousef_shtiwe_cli_core/main.py gateway",
        "yousef_shtiwe_cli_core/main.py --profile",
        "yousef_shtiwe_cli_core/main.py -p",
        "yousef gateway",
        "gateway/run.py",
    ]
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"YOUSEF_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "YOUSEF_HOME=" in command and f"YOUSEF_HOME={current_home}" not in command:
            return False
        return True
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (YOUSEF_HOME-scoped).
...
from yousef_constants import is_container, is_termux, is_wsl
...
SERVICE_DESCRIPTION = "YOUSEF SHTIWE Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current YOUSEF_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (YOUSEF_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary YOUSEF_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when YOUSEF_HOME is a named profile.

    For ``~/.yousef/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit YOUSEF_HOME path. Defaults to the current
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this YOUSEF_HOME.

    Default ``~/.yousef`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.yousef/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other YOUSEF_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    yousef gateway uninstall")
    print_info("    sudo yousef gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            ...
            if run_as_user:
                print_info(f"  After setup, run: sudo yousef gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo yousef gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo yousef gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.yousef`` → ``ai.yousef.gateway.plist`` (backward compatible).
    Profile ``~/.yousef/profiles/coder`` → ``ai.yousef.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.yousef.gateway-{suffix}" if suffix else "ai.yousef.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.yousef/yousef-sovereign-core  -> /home/alice/.yousef/yousef-sovereign-core
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current YOUSEF_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hermes_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.yousef                    → /home/alice/.yousef
      /root/.yousef/profiles/coder     → /home/alice/.yousef/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_hermes_home().resolve()
    current_default = (Path.home() / ".yousef").resolve()
    target_default = Path(target_home_dir) / ".yousef"

    # Default ~/.yousef → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.yousef → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.yousef) — keep as-is
...
User={username}
Group={group_name}
ExecStart={python_path} -m yousef_shtiwe_cli_core.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="YOUSEF_HOME={hermes_home}"
Restart=on-failure
RestartSec=30
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}yousef gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}yousef gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        print(
            ...
            f"  Check status: {'sudo ' if system else ''}yousef gateway status\n"
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}yousef gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}yousef gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}yousef gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.yousef.gateway-{suffix}" if suffix else "ai.yousef.gateway"
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>yousef_shtiwe_cli_core.main</string>",
    ]
    if profile_arg:
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  yousef gateway status             # Check status")
...
def launchd_stop():
    ...
    # `yousef gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("  Run: yousef gateway start")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ☠ WORM V2 Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your YOUSEF SHTIWE",
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info("  Skipped — configure later with 'yousef gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from yousef_shtiwe_cli_core.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from yousef_shtiwe_cli_core.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print_info("  3. Hermes will store the returned account_id/token in ~/.yousef/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `yousef gateway setup`.")
        return
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"YousefShtiwe\"")
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: yousef gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  yousef gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: yousef gateway run")
                print_info("  For persistence:   tmux new -s hermes 'yousef gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: yousef gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup yousef gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: yousef gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'yousef gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: yousef gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: yousef gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s hermes 'yousef gateway run'")
                print()
            ...
            print()
            print("  yousef gateway run                              # direct foreground")
            print("  tmux new -s hermes 'yousef gateway run'         # persistent via tmux")
            print("  nohup yousef gateway run > ~/.yousef/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: yousef gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: yousef gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: yousef gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: yousef gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: yousef gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  yousef gateway install")
                    print("  sudo yousef gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  yousef gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup yousef gateway run > ~/.yousef/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'yousef gateway run'         # persistent via tmux")
                    print("  nohup yousef gateway run > ~/.yousef/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  yousef gateway install  # Install as user service")
                    print("  sudo yousef gateway install --system  # Install as boot-time system service")
