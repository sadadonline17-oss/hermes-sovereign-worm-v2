...
Diagnoses issues with YOUSEF SHTIWE setup.
"""
...
from yousef_shtiwe_cli_core.config import get_project_root, get_hermes_home, get_env_path
from yousef_constants import display_hermes_home

PROJECT_ROOT = get_project_root()
YOUSEF_HOME = get_hermes_home()
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.yousef or ~/.yousef/profiles/coder)

# Load environment variables from ~/.yousef/.env so API key checks work
...
from yousef_constants import OPENROUTER_MODELS_URL
...
from yousef_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.yousef/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # Check ~/.yousef/.env (primary location for user config)
    env_path = YOUSEF_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'yousef setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'yousef setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'yousef setup' to create one")
                issues.append("Run 'yousef setup' to create .env")
    
    # Check ~/.yousef/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = YOUSEF_HOME / 'config.yaml'
    ...
    if config_path.exists():
        try:
            from yousef_shtiwe_cli_core.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'yousef setup' to migrate config")
                else:
                    issues.append("Run 'yousef doctor --fix' or 'yousef setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'yousef doctor --fix'")
        except Exception:
            pass

        # Validate config structure (catches malformed custom_providers, etc.)
        try:
            from yousef_shtiwe_cli_core.config import validate_config_structure
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'yousef doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "yousef"
        ...
        _cmd_link = _cmd_link_dir / "yousef"
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    if should_fix:
                        ...
                        issues.append(f"Broken symlink at {_cmd_link_display}/hermes — run 'yousef doctor --fix'")
                ...
                if should_fix:
                    ...
                    issues.append(f"Missing {_cmd_link_display}/hermes symlink — run 'yousef doctor --fix'")
    ...
    try:
        ...
        if has_aws_credentials():
            ...
            try:
                ...
                print(f"\r  {color('⚠', Colors.YELLOW)} {_label} {color('(boto3 not installed — pip install yousef-sovereign-core[bedrock])', Colors.DIM)}           ")
                issues.append("Install boto3 for Bedrock: pip install yousef-sovereign-core[bedrock]")
            except Exception as _e:
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'yousef setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = YOUSEF_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    from yousef_shtiwe_cli_core.config import get_env_value
    ...
    try:
        ...
        _mem_cfg_path = YOUSEF_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run yousef memory setup)")
    ...
    # =========================================================================
    try:
        from yousef_shtiwe_cli_core.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'yousef doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
