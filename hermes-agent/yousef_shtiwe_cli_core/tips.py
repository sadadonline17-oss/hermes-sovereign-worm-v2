...
TIPS = [
    ...
    "yousef chat -t web,terminal enables only specific toolsets for a focused session.",
    "yousef chat -s github-pr-workflow preloads a skill at launch.",
    "yousef chat -q \"query\" runs a single non-interactive query and exits.",
    "yousef chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "yousef chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "yousef chat --source telegram tags the session for filtering in hermes sessions list.",
    "hermes -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "yousef doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hermes dump outputs a compact setup summary — great for bug reports.",
    "yousef config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "yousef config edit opens config.yaml in your default editor.",
    "yousef config check scans for missing or stale configuration options.",
    ...
    "hermes backup creates a zip backup of your entire Sovereign home directory.",
    ...
    "yousef gateway install sets up Hermes as a system service (systemd/launchd).",
    ...
    "The plan skill saves markdown plans under .yousef/plans/ in the active workspace.",
    ...
    "yousef gateway install sets it up as a system service that starts on boot.",
    ...
    "Context files (.yousef.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.yousef/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Hermes loads project context from .yousef.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "Checkpoints use shadow repos in ~/.yousef/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "yousef chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "yousef shtiwe plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "BOOT.md at ~/.yousef/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.yousef/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.yousef/SOUL.md to customize.",
    ...
    "The skills audit log at ~/.yousef/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at YOUSEF_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "YOUSEF_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in YOUSEF_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.yousef/interrupt_debug.log with timestamps.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.yousef/.env.",
    "Skills from trusted repos (SovereignCore) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.yousef/skills/.hub/quarantine/ holds skills pending security review.",
]
