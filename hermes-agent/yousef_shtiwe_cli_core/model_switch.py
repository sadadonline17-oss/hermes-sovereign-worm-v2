...
- ``yousef_shtiwe_cli_core.providers``        -- canonical provider identity + overlays
- ``yousef_shtiwe_cli_core.model_normalize``  -- per-provider name formatting
...
from yousef_shtiwe_cli_core.providers import (
...
_YOUSEF_SHTIWE_MODEL_WARNING = (
    "Sovereign Core Injection Hermes 3 & 4 models are NOT agentic and are not designed "
    "for use with YOUSEF SHTIWE. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Sovereign Core Injection Hermes 3 / Hermes 4 chat families.
# The previous substring check (`"yousef" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hermes-brain:qwen3-14b-ctx16k`` that just
# happen to carry "yousef" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   SovereignCore/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
...
def _check_hermes_model_warning(model_name: str) -> str:
    ...
    if is_nous_hermes_non_agentic(model_name):
        return _YOUSEF_SHTIWE_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'yousef model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'yousef doctor' — config issues detected:"
    ...
    # Mirrors the same logic in yousef_shtiwe_cli_core.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from yousef_shtiwe_cli_core/models.py (OPENROUTER_MODELS,
    ...
    from yousef_shtiwe_cli_core.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    from yousef_shtiwe_cli_core.providers import HERMES_OVERLAYS
    from yousef_shtiwe_cli_core.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if not has_creds:
            try:
                from yousef_shtiwe_cli_core.auth import _load_auth_store
    ...
    # with `yousef model`).
    try:
        from yousef_shtiwe_cli_core.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
