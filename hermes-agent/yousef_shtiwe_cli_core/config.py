"""
Configuration management for YOUSEF SHTIWE.

Config files are stored in ~/.yousef/ for easy access:
- ~/.yousef/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.yousef/.env         - API keys and secrets

This module provides:
- yousef config          - Show current configuration
- yousef config edit     - Open config in editor
- yousef config set      - Set a specific value
- yousef config wizard   - Re-run setup wizard
"""
...
def is_managed() -> bool:
    ...
    or a .managed marker file in YOUSEF_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade yousef-sovereign-core"
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            ...
            "Edit services.yousef-sovereign-core.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            ...
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade yousef-sovereign-core"
        )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from YOUSEF_HOME/.container-mode.
    ...
    container_name = info.get("container_name", "yousef-sovereign-core")
    exec_user = info.get("exec_user", "yousef")
...
# Re-export from yousef_constants — canonical definition lives there.
from yousef_constants import get_hermes_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    The mode can be overridden via the YOUSEF_HOME_MODE environment variable
    (e.g. YOUSEF_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse YOUSEF_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("YOUSEF_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into YOUSEF_HOME if the user doesn't have one yet."""
...
def ensure_hermes_home():
    """Ensure ~/.yousef directory structure exists with secure permissions.
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"YOUSEF_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.yousef/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "☠ *YOUSEF SHTIWE*" header.
    ...
    # Logging — controls file logging to ~/.yousef/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'yousef-sovereign-core' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.yousef.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'yousef setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'yousef doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("YOUSEF_HOME", "~/.yousef")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: yousef config set <key> <value>")
    ...
    # metadata.yousef.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.yousef/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.yousef/config.yaml."""
    import copy
    ensure_hermes_home()
    config_path = get_config_path()
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.yousef/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.yousef/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.yousef/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.yousef/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.yousef/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.yousef/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.yousef/.env or environment."""
...
def show_config():
    ...
    print(color("│              ☠ WORM V2 Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  yousef config edit     # Edit config file", Colors.DIM))
    print(color("  yousef config set <key> <value>", Colors.DIM))
    print(color("  yousef setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: yousef config set <key> <value>")
            print()
            print("Examples:")
            print("  yousef config set model anthropic/claude-sonnet-4")
            print("  yousef config set terminal.backend docker")
            print("  yousef config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'yousef config migrate' to add them")
        ...
        print("  yousef config           Show current configuration")
        print("  yousef config edit      Open config in editor")
        print("  yousef config set <key> <value>   Set a config value")
        print("  yousef config check     Check for missing/outdated config")
        print("  yousef config migrate   Update config with new options")
        print("  yousef config path      Show config file path")
        print("  yousef config env-path  Show .env file path")
        sys.exit(1)
