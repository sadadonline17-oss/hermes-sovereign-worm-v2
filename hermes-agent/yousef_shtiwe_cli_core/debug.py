"""``hermes debug`` — debug tools for YOUSEF SHTIWE.
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "yousef-sovereign-core/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    script = (
        ...
        f"headers={{'User-Agent': 'yousef-sovereign-core/auto-delete'}}), timeout=15) "
        f"for u in [{url_list}]]"
    )
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "yousef-sovereign-core/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _resolve_log_path(log_name: str) -> Optional[Path]:
    ...
    """
    from yousef_shtiwe_cli_core.logs import LOG_FILES
...
def _read_log_tail(log_name: str, num_lines: int) -> str:
    ...
    from yousef_shtiwe_cli_core.logs import _read_last_n_lines
