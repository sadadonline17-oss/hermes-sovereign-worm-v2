"""
YOUSEF SHTIWE Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.yousef/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing yousef-sovereign-core or hermes PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# YOUSEF SHTIWE" comment and following line
                if '# YOUSEF SHTIWE' in line or '# yousef-sovereign-core' in line:
                    skip_next = True
                    continue
                if skip_next and ('yousef' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hermes
                if 'yousef' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains yousef_shtiwe_cli_core reference)
                content = wrapper.read_text()
                if 'yousef_shtiwe_cli_core' in content or 'yousef-sovereign-core' in content:
                    wrapper.unlink()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.yousef/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.yousef/ for future reinstall
    """
    ...
    print(color("│            ☠ WORM V2-Sovereign Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.yousef/, just remove the yousef-sovereign-core subdir
    ...
    # 5. Optionally remove ~/.yousef/ data directory
    if full_uninstall:
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/SovereignCore/yousef-sovereign-core/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using YOUSEF SHTIWE! ☠")
    print()
