...
`hermes backup` creates a zip archive of the entire ~/.yousef/ directory
(excluding the yousef-sovereign-core repo and transient files).
...
from yousef_constants import get_default_hermes_root, get_hermes_home, display_hermes_home
...
_EXCLUDED_DIRS = {
    "yousef-sovereign-core",     # the codebase repo — re-clone instead
...
def run_backup(args) -> None:
    """Create a zip backup of the Sovereign home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Sovereign home directory not found at {hermes_root}")
        sys.exit(1)
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.yousef/config.yaml` instead of `config.yaml`.
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # yousef_shtiwe_cli_core.profiles might not be available (fresh install)
        ...
        # Guidance
        print()
        if not (hermes_root / "yousef-sovereign-core").is_dir():
            print("Note: The yousef-sovereign-core codebase was not included in the backup.")
...
# Critical state files to include in quick snapshots (relative to YOUSEF_HOME).
