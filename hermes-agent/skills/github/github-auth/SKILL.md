...
- Give it a name like "yousef-sovereign-core"
- Select scopes:
...
- Give it a title like "yousef-sovereign-core-<machine-name>"
...
elif [ -f ~/.yousef/.env ] && grep -q "^GITHUB_TOKEN=" ~/.yousef/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.yousef/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
